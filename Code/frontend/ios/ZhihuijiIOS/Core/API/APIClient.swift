...
final class APIClient {
    ...
    func fetchSaleOrderReceiptPdf(id: EntityID) async throws -> Data {
        let (data, response) = try await performBinaryRequest(
            path: "/v2/sale-orders/\(id.rawValue)/receipt.pdf",
            accept: "application/pdf"
        )
        try validate(response: response, data: data)
        guard let http = response as? HTTPURLResponse,
              http.value(forHTTPHeaderField: "Content-Type")?.localizedCaseInsensitiveContains("application/pdf") == true else {
    ...
    private func performBinaryRequest(path: String, accept: String) async throws -> (Data, URLResponse) {
        var request = try makeBaseRequest(path: path, method: "GET", queryItems: [])
        ...
        if let http = response as? HTTPURLResponse,
           http.statusCode == 401 {
            try await refreshAccessTokenIfNeeded()
            var retriedRequest = try makeBaseRequest(path: path, method: "GET", queryItems: [])
            retriedRequest.setValue(accept, forHTTPHeaderField: "Accept")
            return try await session.data(for: retriedRequest)
        }
