...
export async function fetchSaleOrderReceiptPdf(token: string, id: EntityId) {
  return requestBlob(`/v2/sale-orders/${id}/receipt.pdf`, {
    headers: {
      ...authHeaders(token),
...
async function requestBlob(path: string, init: RequestInit = {}, hasRetriedAuth = false): Promise<Blob> {
  ...
  if (response.status === 401 && !hasRetriedAuth && hasAuthHeader) {
    const refreshedToken = await tryRefreshAccessToken()
    if (refreshedToken) {
      return requestBlob(path, withAuthorization(init, refreshedToken), true)
    }
    emitApiAuthEvent(401)
  } else if (response.status === 403 && hasAuthHeader) {
    emitApiAuthEvent(403)
  ...
  if (!response.ok) {
    const rawText = await response.text()
    const payload = rawText ? safeParse<never>(rawText) : null
    throw new ApiError(payload?.message || `request failed: ${response.status}`, response.status, payload?.code ?? -1)
  }

  const blob = await response.blob()
  if (blob.size === 0) {
    throw new ApiError('小票 PDF 内容为空', response.status)
  }
  return blob
}
