...
const trendDataset = computed(() => trendPoints.value)
...
function buildAlerts() {
  ...
  const advice = workbench.value?.quickQuestions[0]
  if (advice) {
    alerts.push({
      id: 'suggestion',
      tone: 'info',
...
function buildTrendGeometry(points: SalesTrendPoint[]) {
  const safePoints = points.length > 0
    ? points
    : [{ startAt: periodRange.value.startAt, endAt: periodRange.value.endAt, totalSalesAmount: 0, totalOrderCount: 0 }]
...
<template>
  <section class="dashboard-page">
    ...
    <p v-if="loading" class="dashboard-banner dashboard-banner--soft">正在加载经营看板...</p>
    ...
    <section class="dashboard-grid dashboard-grid--top">
      <article class="dashboard-panel dashboard-trend">
        ...
        <div v-if="trendDataset.length" class="trend-wrap">
        ...
        <p v-else class="dashboard-empty-copy">当前周期没有销售趋势数据</p>
        <div v-if="trendDataset.length" class="trend-footer">
