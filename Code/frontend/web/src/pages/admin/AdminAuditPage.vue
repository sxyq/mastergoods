...
async function download(job: AdminExportJob) {
  ...
  if (job.expiresAt && Date.parse(job.expiresAt) <= Date.now()) { exportError.value = new Error('导出任务已过期'); return }
  ...
  try {
    ...
    link.href = url
    link.download = `admin-export-${job.exportId}.csv`
    document.body.appendChild(link)
    link.click()
    link.remove()
    URL.revokeObjectURL(url)
    exportNotice.value = '导出文件已下载'
    await loadExports()
  } catch (cause) { exportError.value = cause }
