...
class AgentMarkdownTextParserTest {
    @Test
    fun parseTableKeepsEscapedPipeInsideCell() {
        val table = parseMarkdownBlocks(
            "| 商品 | 规格 A\\|B | 金额 |\n| --- | --- | --- |"
        ).single()

        assertEquals(listOf("商品", "规格 A|B", "金额"), tableHeaders(table))
    }

    @Test
    fun parseTableKeepsPipeInsideInlineCode() {
        val table = parseMarkdownBlocks(
            "| 表达式 | `a\\|b` | 说明 |\n| --- | --- | --- |"
        ).single()

        assertEquals(listOf("表达式", "`a|b`", "说明"), tableHeaders(table))
    }
    ...
    @Suppress("UNCHECKED_CAST")
    private fun tableHeaders(block: Any): List<String> {
        val headersProperty = block.javaClass.getDeclaredMethod("getHeaders")
        headersProperty.isAccessible = true
        return headersProperty.invoke(block) as List<String>
    }
