...
class AgentChatViewModelAnswerMergeTest {
    ...
    @Test
    fun blockedRunCompletionKeepsAssistantMessageAndShowsSafeMessage() {
        val message = chatMessage(
            id = "assistant",
            content = "",
            isStreaming = true,
        ...
        val completed = message.withRunCompletedState(
            finalAnswer = null,
            safeMessage = "当前请求无法完成，请调整后重试。",
        )

        assertEquals("当前请求无法完成，请调整后重试。", completed.content)
        assertEquals(
            listOf(ChatMessagePart.Text("当前请求无法完成，请调整后重试。")),
            completed.parts,
        )
