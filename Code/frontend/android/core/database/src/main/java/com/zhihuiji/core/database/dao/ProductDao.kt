...
interface ProductDao {
    ...
    @Query("SELECT * FROM products WHERE stock < safeStock ORDER BY (safeStock - stock) DESC LIMIT :limit")
    fun observeLowStock(limit: Int): Flow<List<ProductEntity>>

    @Query("SELECT * FROM products WHERE id = :id")
    suspend fun findById(id: Long): ProductEntity?

    @Query("SELECT * FROM products WHERE code = :code AND id < 0 ORDER BY updatedAt DESC LIMIT 1")
    suspend fun findTemporaryByCode(code: String): ProductEntity?
