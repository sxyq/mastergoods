...
class SettingsStore @Inject constructor(
    ...
    companion object {
        ...
        const val DEFAULT_BASE_URL = "https://zhj-api.sxyq27.online/"
        private const val PRODUCTION_HOST = "zhj-api.sxyq27.online"
        ...
        internal fun normalizeBaseUrl(raw: String, allowDebug117Host: Boolean): String {
            ...
            if (isLegacyEdgeBaseUrl(withTrailingSlash)) return DEFAULT_BASE_URL
            return stripEndpointVersionSuffix(withTrailingSlash)
        }

        private fun isLegacyEdgeBaseUrl(baseUrl: String): Boolean {
            val uri = runCatching { URI(baseUrl) }.getOrNull() ?: return false
            val path = uri.path.orEmpty().trimEnd('/')
            return uri.host.equals("sxyq27.online", ignoreCase = true) &&
                path in setOf("/zhj-api", "/zhj-api/v1", "/zhj-api/v2")
        }
