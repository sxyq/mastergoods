    ...
    public ApiResponse<V2SyncDtos.SyncUploadResponse> upload(@Valid @RequestBody V2SyncDtos.SyncUploadRequest request) {
        ...
        return ApiResponse.success(new V2SyncDtos.SyncUploadResponse(
            ...
            result.failures().stream()
                .map(item -> new V2SyncDtos.SyncOperationFailureResponse(
                    item.operationId(),
                    item.code(),
                    item.message()
                ))
                .toList(),
            result.operationResults().stream()
                .map(item -> new V2SyncDtos.SyncOperationResultResponse(
                    item.operationId(),
                    item.status(),
                    item.code(),
                    item.message(),
