...
public interface FinanceRecordRepository extends JpaRepository<FinanceRecordEntity, Long> {
    ...
    @Query("SELECT e FROM FinanceRecordEntity e WHERE e.id = :id AND e.ownerUserId = :ownerUserId")
    java.util.Optional<FinanceRecordEntity> findByIdAndOwnerUserIdForUpdate(
        @Param("id") Long id,
