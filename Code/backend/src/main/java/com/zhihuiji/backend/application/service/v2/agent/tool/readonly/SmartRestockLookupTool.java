...
public class SmartRestockLookupTool extends ToolSupport {
    ...
    public String description() {
        return "结合低库存商品与近30天销售给出补货建议量、建议数量与紧急度；用户提到补货、该补多少或哪些要马上补时必须调用本工具，库存全景工具不能替代它";
    }
    ...
    public JsonNode parameterSchema() {
        ...
        categoryId.put("description", "商品分类 ID（可选，用于过滤指定分类）；用户未指定分类时不要猜测");
        ObjectNode limit = properties.putObject("limit");
        limit.put("type", "integer");
        limit.put("description", "返回补货建议条数上限，默认 20");
        return schema;
    }
