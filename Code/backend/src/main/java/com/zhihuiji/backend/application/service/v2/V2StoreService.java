...
public class V2StoreService {
    ...
    public V2StoreDtos.CurrentStoreResponse getCurrentStore() {
        ...
        List<StoreMembershipEntity> memberships = storeMembershipRepository
            .findByOwnerUserIdAndStoreIdOrderByCreatedAtAsc(ownerUserId, context.store().getId());
        int enabledCount = 0;
    ...
    public V2StoreDtos.MemberResponse updateMember(Long userId, V2StoreDtos.MemberUpdateRequest request) {
        ...
        StoreContext context = ensureStoreContext(ownerUserId);
        ensureCurrentUserCanManage(context);
        StoreMembershipEntity membership = storeMembershipRepository
            .findByOwnerUserIdAndStoreIdAndUserId(ownerUserId, context.store().getId(), userId)
    ...
    public StoreContext ensureStoreContext(Long ownerUserId) {
        ...
        StoreMembershipEntity ownerMembership = storeMembershipRepository
            .findByOwnerUserIdAndStoreIdAndUserId(ownerUserId, store.getId(), ownerUserId)
    ...
    private StoreMembershipEntity resolveCurrentMembership(StoreContext context, Long currentUserId) {
        return storeMembershipRepository.findByOwnerUserIdAndStoreIdAndUserId(
                context.store().getOwnerUserId(), context.store().getId(), currentUserId)
            .orElseGet(() -> {
