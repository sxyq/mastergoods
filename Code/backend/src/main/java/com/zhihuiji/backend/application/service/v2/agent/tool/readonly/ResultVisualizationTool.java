...
public class ResultVisualizationTool extends ToolSupport {
    ...
    public String description() {
        return "已经拿到真实查询结果后，由模型判断表格、图、统计卡、趋势或排行是否更容易让用户理解；"
            + "普通文字已经足够时不要调用。本工具只决定展示方式，不查询业务数据，必须和真实查询工具一起使用";
    }
    ...
    public ToolResult execute(ToolContext ctx, JsonNode params) {
        ...
        if (mode == null || mode.isBlank() || !Set.of("auto", "table", "chart", "kpi", "timeline").contains(mode.toLowerCase())) {
            mode = "auto";
        } else {
