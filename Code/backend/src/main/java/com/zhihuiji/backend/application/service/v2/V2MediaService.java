...
public class V2MediaService {
    ...
    public ResponseEntity<byte[]> readAssetContent(Long id) {
        ...
        try {
            ...
            String mime = entity.getMimeType() == null ? "" : entity.getMimeType().toLowerCase(Locale.ROOT);
            MediaType contentType = ALLOWED_MIME_TYPES.contains(mime)
    ...
    public V2MediaDtos.MediaAssetResponse createAsset(V2MediaDtos.MediaAssetCreateRequest request) {
        ...
        validateAssetMetadata(entity.getOriginalFileName(), entity.getMimeType(), request.sizeBytes());
    ...
    public V2MediaDtos.MediaAssetResponse uploadFile(MultipartFile file, String assetType) throws Exception {
        ...
        validateAssetMetadata(file.getOriginalFilename(), file.getContentType(), file.getSize());
    ...
    public void deleteAsset(Long id) {
        ...
        try {
            mediaStorageService.delete(entity.getObjectKey());
        } catch (Exception exception) {
            throw new IllegalStateException("删除媒体文件失败: " + id, exception);
        }
    }
    ...
    private void validateAssetMetadata(String originalFileName, String mimeType, Long sizeBytes) {
        String normalizedName = normalizeRequired(originalFileName, "originalFileName 不能为空");
        String normalizedMime = normalizeRequired(mimeType, "mimeType 不能为空").toLowerCase(Locale.ROOT);
        if (normalizedName.length() > MAX_FILE_NAME_LENGTH) {
            throw new IllegalArgumentException("文件名过长");
        }
        if (!ALLOWED_MIME_TYPES.contains(normalizedMime)) {
            throw new IllegalArgumentException("不支持的文件类型");
        }
        if (sizeBytes == null || sizeBytes <= 0L || sizeBytes > MAX_UPLOAD_SIZE_BYTES) {
