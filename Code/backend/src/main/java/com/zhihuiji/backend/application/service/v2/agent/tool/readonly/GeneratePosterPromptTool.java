...
public class GeneratePosterPromptTool extends ToolSupport {
    ...
    public String description() {
        return "根据真实商品信息与用户意图，生成优化的海报提示词（含风格/构图/文案建议），不直接生成图片。必须先拿到真实 product_id；如果用户只说了商品名称或没有提供编号，先查询商品，再用查询结果中的 product_id 调用本工具，不能直接凭商品名称编写内容";
    }
    ...
    public JsonNode parameterSchema() {
        ...
        addStringProperty(schema, "intent", "海报用途或文案意图，可选");
        return addRequired(schema, "product_id");
    }
    @Override
    public ToolResult execute(ToolContext ctx, JsonNode params) {
        ...
        return ToolResult.success(List.of(promptBlock), toolFacts, toolSummary);
    }
