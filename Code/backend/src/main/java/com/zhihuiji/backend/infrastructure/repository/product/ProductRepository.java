...
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {
    ...
    List<ProductEntity> findAllByOwnerUserIdOrderByUpdatedAtDescIdDesc(Long ownerUserId);

    List<ProductEntity> findAllByOwnerUserId(Long ownerUserId);

    @Query("SELECT e FROM ProductEntity e WHERE e.ownerUserId = :ownerUserId AND COALESCE(e.updatedAt, e.createdAt) >= :sinceTimestamp ORDER BY COALESCE(e.updatedAt, e.createdAt) ASC, e.id ASC")
    ...
    @Query("SELECT p FROM ProductEntity p WHERE p.ownerUserId = :ownerUserId AND p.stock <= p.safeStock ORDER BY p.stock ASC, p.updatedAt DESC, p.id DESC")
