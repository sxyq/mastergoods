...
public class SalesReturnLookupTool extends ToolSupport {
    ...
    public JsonNode parameterSchema() {
        ...
        addStringProperty(schema, "keyword", "销售退货单关键词，可选");
        addIntegerProperty(schema, "status", "销售退货单状态，可选");
        addIntegerProperty(schema, "original_order_id", "原销售单 ID，可选");
        return schema;
    }
    @Override
    public ToolResult execute(ToolContext ctx, JsonNode params) {
        Long ownerUserId = ctx.ownerUserId();
        // PostgreSQL may infer a null CONCAT parameter as bytea. Empty text
        // preserves the same unfiltered query semantics with a stable type.
