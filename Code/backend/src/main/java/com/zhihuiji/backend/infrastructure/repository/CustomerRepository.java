...
public interface CustomerRepository extends JpaRepository<CustomerEntity, Long> {
    ...
    @Query("SELECT e FROM CustomerEntity e WHERE e.ownerUserId = :ownerUserId AND COALESCE(e.updatedAt, e.createdAt) >= :sinceTimestamp ORDER BY COALESCE(e.updatedAt, e.createdAt) ASC, e.id ASC")
    ...
    @Query("SELECT e FROM CustomerEntity e WHERE e.id = :id AND e.ownerUserId = :ownerUserId")
    Optional<CustomerEntity> findByIdAndOwnerUserIdForUpdate(
        @Param("id") Long id,
        @Param("ownerUserId") Long ownerUserId
    );
}
