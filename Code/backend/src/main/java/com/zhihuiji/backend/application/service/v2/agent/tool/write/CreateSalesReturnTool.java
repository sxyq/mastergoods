...
public class CreateSalesReturnTool extends ToolSupport {
    ...
    public JsonNode parameterSchema() {
        ...
        addIntegerProperty(schema, "sale_order_id", "销售单 ID");
        addStringProperty(schema, "reason", "退货原因");
        addArrayProperty(schema, "items", "销售退货商品明细", returnItemSchema(), 1);
        addRequired(schema, "sale_order_id", "items");
        return schema;
    }
    ...
    public ToolResult execute(ToolContext ctx, JsonNode params) {
        ...
        if (referenceValidator != null
            && (!referenceValidator.saleOrderBelongsToOwner(ctx.ownerUserId(), saleOrderId)
                || !hasOwnedProducts(ctx, params))) {
            String error = "销售单或商品不属于当前账号，无法生成退货草稿";
