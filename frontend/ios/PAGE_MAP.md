...
- 路由与会话：`ZhihuijiIOS/App/AppRouter.swift`、`ZhihuijiIOS/App/AppSession.swift`、`ZhihuijiIOS/App/AppEnvironment.swift`
- API 层：`ZhihuijiIOS/Core/API/*`
- 设计系统：`ZhihuijiIOS/Core/Design/*`
- 业务页面：`ZhihuijiIOS/Features/*`
...
- 代码级验证：`xcrun swiftc -typecheck` 对 `ios/ZhihuijiIOS` 全量 App 源码通过
- 临时模块验证：`swiftc -emit-module -enable-testing -module-name ZhihuijiIOS` 通过
- 工程引用：`ios/ZhihuijiIOS.xcodeproj/project.pbxproj` 与现有 Swift 文件已对齐
- 工程文件验证：`plutil -lint ios/ZhihuijiIOS.xcodeproj/project.pbxproj` 通过
- 收口验证：`InlinePermissionAuditTests` 已覆盖 `ios/ZhihuijiIOS` 源码，当前不存在 `session.hasPermission(...)` 形式的 inline 权限判断
