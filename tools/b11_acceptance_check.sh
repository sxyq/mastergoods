...
ANDROID_DIR="${ROOT_DIR}/master-goods-android"
JDK21_HOME="${JDK21_HOME:-/Users/sunyiyang/.local/jdks/temurin-21/Contents/Home}"
EVIDENCE_ROOT="${EVIDENCE_ROOT:-${ROOT_DIR}/docs/acceptance-evidence/b11}"
...
backend_smoke() {
  ...
  if [[ ! -x "${gradle_cmd}" ]]; then
    if [[ -x "${ANDROID_DIR}/gradlew" ]]; then
      ...
      echo "BLOCKED: no backend ./gradlew, no reusable Android gradle wrapper, and no global gradle command found." >&2
      exit 127
    fi
  fi
