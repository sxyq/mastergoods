...
TRACKED_EXACT = {
    ...
    "web/vite.config.ts",
    "web/index.html",
    "master-goods-android/build.gradle.kts",
    "master-goods-android/settings.gradle.kts",
    "master-goods-android/gradle.properties",
}
...
def subsystem_for(path: str) -> str:
    if path.startswith("master-goods-android/"):
        return "android"
    if path.startswith("web/") or path.startswith("src/main/resources/static/admin-console/"):
        return "web"
    if path.startswith("src/") or path in {"build.gradle.kts", "settings.gradle.kts", "gradle.properties"}:
        return "backend"
    return "other"
...
def load_index(project_root: Path) -> tuple[dict[str, dict[str, object]], list[dict[str, object]]]:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for row in conn.execute(
            ...
            from files
            where path like 'src/main/java/%'
               or path like 'src/main/resources/%'
               or path like 'src/test/java/%'
               or path like 'src/test/resources/%'
               or path like 'src/main/resources/static/admin-console/%'
               or path like 'master-goods-android/%'
               or path like 'web/src/%'
               or path = 'web/vite.config.ts'
               or path = 'build.gradle.kts'
               or path = 'settings.gradle.kts'
            order by path
            """
        ):
        ...
        for row in conn.execute(
            ...
            where kind in ({placeholders})
              and (
                file_path like 'src/main/java/%'
                or file_path like 'src/main/resources/%'
                or file_path like 'src/test/java/%'
                or file_path like 'src/test/resources/%'
                or file_path like 'src/main/resources/static/admin-console/%'
                or file_path like 'master-goods-android/%'
                or file_path like 'web/src/%'
                or file_path = 'web/vite.config.ts'
                or file_path = 'build.gradle.kts'
                or file_path = 'settings.gradle.kts'
              )
