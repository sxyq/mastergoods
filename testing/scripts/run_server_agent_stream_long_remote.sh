...
RUN_ID="${RUN_ID:-agent-stream-long-$(date +%Y%m%dT%H%M%S%z)}"
REMOTE_DIR="${REMOTE_DIR:-/tmp/${RUN_ID}}"
LOCAL_SCRIPT="${ROOT_DIR}/testing/scripts/run_server_agent_stream_long.py"
mkdir -p "${EVIDENCE_ROOT}"
export ROOT_DIR EVIDENCE_ROOT SSH_KEY JUMP_HOST TARGET_HOST SSH_PASSWORD RUN_ID REMOTE_DIR LOCAL_SCRIPT
...
spawn scp -q -o StrictHostKeyChecking=accept-new -o "$proxy" $local_script root@$target:$remote_dir/run_server_agent_stream_long.py
expect {
    -re {[Pp]assword:} { send -- "$env(SSH_PASSWORD)\r"; exp_continue }
    -re {Permission denied} { exit 13 }
    eof { }
...
set remote_command [format {set +e; : > '%s/provider.log'; (docker logs -f --since 1s zhihuiji154-backend > '%s/provider.log' 2>&1) & log_pid=\$!; export BASE_URL='http://127.0.0.1:18080'; export OUTPUT_ROOT='%s/evidence'; export DB_CONTAINER='zhihuiji154-postgres'; export DB_USER='zhihuiji'; export DB_NAME='zhihuiji'; export TEST_HOST='154.217.241.207'; export AGENT_LLM_MODEL='deepseek-v4-flash'; export AGENT_LLM_BASE_URL='https://tokenrhythm.studio/v1'; export AGENT_LLM_WIRE_API='chat_completions'; export AGENT_TOOL_CHOICE='auto'; export TURN_GAP_SECONDS='1.5'; export TOKEN=\$(docker exec zhihuiji154-postgres psql -U zhihuiji -d zhihuiji -At -c 'select token from sessions where user_id = 7 and is_active = true and expires_at > (extract(epoch from now()) * 1000)::bigint order by id desc limit 1;' | head -n 1); if [ -z "\$TOKEN" ]; then echo TOKEN_NOT_FOUND > '%s/runner.log'; rc=2; else python3 '%s/run_server_agent_stream_long.py' > '%s/runner.log' 2>&1; rc=\$?; fi; kill \$log_pid 2>/dev/null || true; echo RUNNER_EXIT_CODE=\$rc; exit 0} $remote_dir $remote_dir $remote_dir $remote_dir $remote_dir $remote_dir]
