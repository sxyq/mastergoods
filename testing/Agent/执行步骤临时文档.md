...
| Git 状态 | 分支 `codex/publish-local-updates`，HEAD `1a69b431`；仅保留用户未提交的 `DemoDataService.java` 修改 | 当前工作树 | 所有提交使用路径级暂存，不得重置或混入该文件 |
...
| 19:45–19:50 | D5/D6 | 细化管理员会话续期、权限拒绝处理、运行详情局部重试与代次保护，并补充 Token、请求数和延迟的 bucket 展示 | `Code/frontend/admin-web/src/app/stores/admin-session.ts`；`src/shared/api/client.ts`；`src/pages/LoginPage.vue`；`src/pages/AgentRunsPage.vue`；`src/features/agent-observability/UsageChart.vue` | `PATH=.../node/bin:$PATH VITE_API_BASE_URL=https://zhj-api.sxyq27.online npm run build`：Passed，1634 个模块；`git diff --check`：Passed | `163d726a fix(admin): handle session renewal and forbidden state`；运行详情和用量展示待提交 | 未部署本批 | 续期失败回登录、权限拒绝保留 forbidden、异步详情不串 run；下一步分别提交运行详情和用量展示，并重发管理员 Web |
