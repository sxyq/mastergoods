...
| 项目 | 实际事实 | 状态 |
|---|---|---|
| Android 登录网络边界 | 关联客户端用例 `AG-CLI-AND-P2-LOGIN-001` 的真实 Android App 登录请求已到达 `8.220.206.9` 公网 API，接口返回 HTTP 422。该响应只证明请求到达并收到服务端响应，不能记为 Agent 集成通过。 | `Blocked` |
| 测试账号与执行范围 | 四个指定账号后缀 `8111`、`8112`、`8113`、`8114` 均不存在；未重置密码、未创建账号；未进入 Wave 1-4，也未执行真实写入请求。 | `Blocked` |
