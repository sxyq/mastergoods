...
- 现有用户/门店权限：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/store/`
- 现有权限注解/拦截器：`Code/backend/src/main/java/com/zhihuiji/backend/infrastructure/security/`
- Web 会话权限参考：`Code/frontend/web/src/app/stores/session.ts`

## 对应接口

权限适用于 `API-ADM-01` 至 `API-ADM-15`，接口不能自定义超出本设计的范围。

## 对应测试

覆盖角色正反、owner/store 交叉、资源 ID 猜测、直接 URL、缓存隔离、内容权限、写库为零、审计成功和审计失败等场景。

## 当前限制

正式管理员角色来源、观察员范围授权方式和最后一个超级管理员保护规则尚未在正式代码中确定；当前仅完成设计约束。
