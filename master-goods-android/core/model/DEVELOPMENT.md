...
- Agent（V2，已重构）：`AgentConversationDto`、`AgentMessageDto`、`AgentDraftDto` 及相关请求体（见 `v2/agent/AgentV2Models.kt`）
- Agent（新版流式）：`AgentStreamEvent`、`ChatMessage`、`ResultBlock` 等（见 `v2/agent/AgentStreamModels.kt`、`AgentChatModels.kt`、`AgentResultBlockModels.kt`）
