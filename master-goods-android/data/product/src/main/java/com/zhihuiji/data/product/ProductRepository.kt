...
class ProductRepository @Inject constructor(
    ...
    suspend fun refreshProducts(keyword: String?) {
        ...
        result.onSuccess { products ->
            productDao.upsertAll(products.mapNotNull { it.toEntity() })
        }
    }
    ...
    suspend fun getProduct(id: Long): Result<ProductDto> {
        ...
        remote.onSuccess { it.toEntity()?.let { entity -> productDao.upsert(entity) } }
        return remote.fold(
    ...
    suspend fun createProduct(draft: ProductDraft): Result<ProductDto> =
        safeApiCall { api.createProduct(draft.toCreateRequest()) }.also { result ->
            result.onSuccess { it.toEntity()?.let { entity -> productDao.upsert(entity) } }
        }

    suspend fun updateProduct(id: Long, draft: ProductDraft): Result<ProductDto> =
        safeApiCall { api.updateProduct(id, draft.toUpdateRequest()) }.also { result ->
            result.onSuccess { it.toEntity()?.let { entity -> productDao.upsert(entity) } }
        }
