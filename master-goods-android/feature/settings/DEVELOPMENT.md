...
- 当前状态：设置页主体已接入，当前真实实现包含账号信息、受控服务端地址展示、手动同步、导入任务列表/空态、退出登录；本轮新增“店员与权限”专页，真实对齐 `/v1/admin/users` 的账号/店员创建、昵称修改、启停与密码重置。缓存统计与独立清理入口仍未实现。
...
- “店员与权限”要明确说明：当前是真实 `admin users` 管理，不是完整的 `store/member` 角色绑定体系；后者仍待后端补齐。

## 验收标准

- 设置页能独立承接账号查看、受控服务端信息查看、真实店员账号管理、手动同步、导入任务观察和退出登录。
