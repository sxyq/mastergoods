...
fun SaleOrderDetailScreen(
    ...
    Column(modifier = Modifier.fillMaxSize().glassBackground()) {
        GlassTopBar(
            ...
            actions = {
                IconButton(onClick = {}) {
                    Icon(Icons.Default.Print, contentDescription = "打印", tint = ZhihuijiColors.TextPrimary)
                }
            },
        )
        if (order != null) {
            Column(
                ...
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.padding(horizontal = 12.dp, vertical = 10.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            ...
                            Text("业务员：张三", style = ZhihuijiTypography.bodySmall, color = ZhihuijiColors.TextSecondary)
                        }
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            ...
                            Text("来源：手机开单", style = ZhihuijiTypography.labelSmall, color = ZhihuijiColors.TextTertiary)
                        }
                    }
                }
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                ...
                if (uiState.payments.isEmpty()) {
                    GlassCard(modifier = Modifier.fillMaxWidth()) {
                        Column(modifier = Modifier.padding(horizontal = 12.dp, vertical = 16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                                Text("收款记录", style = ZhihuijiTypography.titleMedium)
                                Text("去收款", style = ZhihuijiTypography.labelMedium, color = ZhihuijiColors.Primary)
                            }
                            Text("暂无收款记录", style = ZhihuijiTypography.bodySmall, color = ZhihuijiColors.TextTertiary, modifier = Modifier.fillMaxWidth())
                        }
                    }
                }
            ...
            BottomActionBar(
                primaryAction = {
                    if (order.paidAmount < order.totalAmount) {
                        PrimaryGradientButton(text = "收款", onClick = { showPaymentSheet = true }, modifier = Modifier.fillMaxWidth())
                    } else {
                        PrimaryGradientButton(text = "完成订单", onClick = { viewModel.completeOrder() }, modifier = Modifier.fillMaxWidth())
                    }
                },
