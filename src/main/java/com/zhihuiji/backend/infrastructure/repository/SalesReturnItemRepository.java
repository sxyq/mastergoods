...
public interface SalesReturnItemRepository extends JpaRepository<SalesReturnItemEntity, Long> {
    ...
    @Query("SELECT e FROM SalesReturnItemEntity e WHERE e.ownerUserId = :ownerUserId AND e.createdAt >= :sinceTimestamp ORDER BY e.createdAt ASC")
    List<SalesReturnItemEntity> findChangedByOwnerUserId(@Param("ownerUserId") Long ownerUserId, @Param("sinceTimestamp") Long sinceTimestamp);
    ...
    List<SalesReturnItemEntity> findByOwnerUserIdAndReturnIdInOrderByReturnIdAscCreatedAtAsc(
        Long ownerUserId,
