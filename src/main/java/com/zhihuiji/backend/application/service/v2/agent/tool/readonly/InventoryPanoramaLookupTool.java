...
public class InventoryPanoramaLookupTool extends ToolSupport {
    ...
    public String description() {
        return "按商品输出当前库存、安全库存、近30天销量、周转天数与建议补货量";
    }
    ...
    public ToolResult execute(ToolContext ctx, JsonNode params) {
        ...
        String answer = "商品「" + focus.productName() + "」当前库存 " + formatNumber(focus.stock())
            + "，安全库存 " + formatNumber(focus.safeStock())
            + "，近30天销量 " + formatNumber(focus.recentSalesQuantity())
            + "，周转天数 " + turnoverText(focus.turnoverDays())
            + "，建议补货量 " + formatNumber(focus.suggestedRestock()) + "。";
