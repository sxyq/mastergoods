...
public class SalesFullChainLookupTool extends ToolSupport {
    ...
    public String description() {
        return "查询销售单及其关联收款记录、退货记录的完整业务链路";
    }
    ...
    public ToolResult execute(ToolContext ctx, JsonNode params) {
        ...
        String answer = "销售单 " + safeText(order.getOrderNo(), "-")
            + "（" + safeText(order.getCustomerName(), "-") + "）总额 " + money(totalAmount)
            + "，已收 " + money(paidAmount) + "，未收 " + money(unpaidAmount)
            + "；关联收款 " + payments.size() + " 条，退货 " + returns.size() + " 条（退货金额 " + money(returnTotal) + "）。";
