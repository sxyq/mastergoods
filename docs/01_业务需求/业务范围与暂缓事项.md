...
| 依据测试 | `testing/README.md`、`testing/Agent/Agent综合功能与性能测试方案.md` |
...
| Agent | 对话、工具、SSE、结果块、草稿、取消、审计 | `V2AgentAiService` + `application/service/v2/agent/` |
...
| 生图 | 生图 provider 未配置 | 配置生图 provider | `testing/已知问题与解除条件.md` #6、#13 |
| 图片输入 | 图片输入链路未启用 | 多模态链路启用 | 8220 基线 |
| 图片结果展示 | 图片结果展示未启用 | 多模态链路启用 | 8220 基线 |
| 会员体系 | 明确不纳入当前 Agent/UI 重构范围 | 后续另行立项 | `docs/README.md`（原说明） |

注意：`AgentImageService`（`POST /v2/agent/images/generate`）与 `LongCatAnthropicClient.ImageInput`、`buildMultimodalDirectAnswer` 源码存在，但属于 **Deferred**，不能写成已完成。
...
- 接口路径：`/v2/*`（当前范围）；`/v2/agent/images/generate`（Deferred，源码存在未启用）
- 请求模型：`api/dto/v2/`
- 响应模型：`api/dto/v2/`
- SSE 事件：Agent 链路事件
...
- 功能测试：`testing/后端/功能测试/TEST_PLAN.md`、`testing/Agent/Agent综合功能与性能测试方案.md`
