...
**性能 JSON 当前数量（2026-09-24，§19 之后）**：`perf-results-v2.json` **177** 条、`perf-summary.json` rows **168** 条、summary **46** 组；其中 §18 截止时为 162 / 153 / 43，§19 新增 15 / 15 / 3。
...
- **同条件复测结果**：**候选改动未产生明确收益，差异方向不一致，且处于运行波动范围内。** 正式对照只认 §18.5 的 `base1` / `cand1` 表；本条**不再重复**任何早期对照数字，避免与 §18.5 混读。该改动**没有**任何一项指标可证明其价值。
...
3. 本轮数据写入**新批次** `2026-09-24-d715a3a4-perfwave`，文件名与前缀 `base1-` / `cand1-` / `before-`（仅上传·打印）/ `*_onedir` **互不冲突**；§18 截止时 `perf-results-v2.json` 95→**162** 条、`perf-summary.json` rows 86→**153** 条、summary 键 **43** 个（41 个汇总组 + 2 个批次说明键）。**当前总计**（含 §19 contentType 批次）为 **177** 条结果、**168** 条 summary rows、**46** 个 summary 分组；其中 §18 截止 162 / 153 / 43，§19 新增 15 / 15 / 3。
