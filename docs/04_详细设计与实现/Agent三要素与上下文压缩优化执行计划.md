...
| 当前变更范围 | 后端 Agent 编排、工具门、上下文、记忆、搜索、跨端契约与测试；真实验证按执行文档推进 |
...
本计划最初用于拆解开发工作。当前源码已经包含部分编排、工具执行门、上下文预算与压缩、记忆、搜索及草稿确认组件；这些代码事实不等于自动化测试通过，也不等于真实服务器和 App 已验收。开发、部署和真实测试按[Agent 开发与真实测试执行步骤](../../testing/Agent/执行步骤临时文档.md)推进。
...
| 中 | 长会话上下文边界需要实测 | 当前源码已有上下文构建、预算和检查点路径，仍需验证历史事实是否完整保留 | 阶段 4 |
| 中 | latestSummary 不能承担完整检查点职责 | 列表摘要只能用于预览；正式检查点必须经过上下文服务和边界校验 | 阶段 4 |
| 中 | context_compacted 的真实端到端行为待验证 | Web、Android、iOS 有事件模型，后端已有压缩服务；仍需验证事件、检查点和降级路径 | 阶段 4 |
| 中 | 长期记忆真实闭环待验证 | 当前源码已有提取、召回、删除和 owner/store 隔离路径，仍需验证跨会话行为 | 阶段 5 |
| 中 | 在线搜索真实 Provider 闭环待验证 | 当前源码已有搜索工具、URL 安全、摘要和引用结构，仍需验证 Provider 与结果展示 | 阶段 6 |
| 待验证 | 同店多成员并发和真实跨店测试不足 | 权限和运行上下文需要真实多账号验证 | 阶段 7 |

### 2.3 重要边界

- 当前客户端已有 context_compacted 事件模型和展示组件，后端已有 `ContextCompactionService` 与检查点路径。计划中仍需把“代码存在”“自动化验证”和“服务端真实压缩”分别验收。
...
| 上下文压缩 | 给模型哪些历史，哪些历史应压缩 | 模型窗口、消息、工具结果、检查点 | 上下文包、检查点、压缩事件 | `ContextCompactionService`、`ContextBuilder` |
...
- [Agent 测试总览](../../testing/Agent/README.md)：本计划新增上下文压缩、工具状态机和真实 Agent 回归矩阵；执行顺序见[Agent 开发与真实测试执行步骤](../../testing/Agent/执行步骤临时文档.md)。
