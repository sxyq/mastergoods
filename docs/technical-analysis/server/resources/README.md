...
- 当前资源：
  ...
  - `db/migration/V1__init.sql` ~ `V12__sync_and_import_owner_upgrade.sql`
...
| B06 同步与导入迁移脚本 | 新版已做 | 旧版没有 server 侧 import job 模型 | 为 owner 私有导入任务提供结构落点 | 已新增 `V12__sync_and_import_owner_upgrade.sql` | 首轮只落 `import_jobs`，执行器后续再补 |
