...
"已有密码验证"适合用户知道旧密码、主动换密码；手机号和邮箱用于忘记密码或无法提供旧密码的情况。
...
# 10. 多 Merchant 安全边界（设计演进记录）

**历史设计（第 38 轮，多 Merchant 模型下）：**

因为当时确认"一个 User 可以属于多个 Merchant"，而 Password Credential 属于 **User**（不是 Membership），存在：
...
> 如果 A 店店长随意重置张三的全局密码，就会影响张三在 B 店登录。
...
sequenceDiagram
    ...
    alt 只有当前 Merchant
        ...
        RESET-->>O: 不允许直接全局重置
        Note over O,RESET: 改走手机号/邮箱自助恢复 或 SYSTEM_ADMIN
    end
```

**图示说明（10. 多 Merchant 安全边界（设计演进记录））：** 这是服务调用时序图，参与者包括 OWNER、Password Reset Service、PostgreSQL。箭头表示一次调用或返回，alt/else 分支表示 Decision 的不同结果；事务提交前只做校验和状态准备，短信、邮件等外部副作用应在提交后通过 Outbox 执行。
...
因此"A 店店长重置员工密码可能影响 B 店 STAFF 身份"这个场景**已经不存在**。

但 User / Membership 仍保持分离，强约束为：
...
# 11. Password Reset Strategy Resolver（封口设计）
...
7. OWNER / STAFF 单店规则下，OWNER 重置不影响其他 Merchant（STAFF 单店）；
...
```text
密码强度具体算法（最低长度 / 熵计算细节）
验证码 TTL 具体值（5 分钟还是 10 分钟，不阻塞模块完成）
临时密码有效期具体时长
Cookie/Session 策略中 Web 端的有效期
```
