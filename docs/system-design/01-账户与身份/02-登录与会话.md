...
Membership ACTIVE
AND
Merchant ACTIVE or FROZEN
```

Merchant 为 `FROZEN` 时，Session 仍可用于登录、查看和导出；经营写操作必须由 Merchant Feature / Command Policy 拒绝。
...
```text
建议商户 Web 使用 7 天滑动 Session；iOS / Android 保持 30 天滑动；SYSTEM_ADMIN Web 使用独立、更短且可配置的 Session Policy。
建议 Token 采用 opaque session token + 服务端 Session；具体编码格式留到实现阶段。
建议首期登录标识只使用 EMAIL；PHONE 保留扩展能力但首期禁用，不用于登录或恢复。
```

以上为建议确认结论，详见 `待确认事项.md` 的 P-07—P-10。
