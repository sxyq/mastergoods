...
| 层 | 作用 | 例子 |
|---|---|---|
| User | 这个人整个账号是否可用 | ACTIVE / DISABLED |
...
**第 30、31 轮（原始设计）：** 底层 Membership 模型支持多 Merchant，第一版 UI 保持单 Merchant 简单体验。

**第 39 轮（STAFF 收紧）：** 按用户要求，STAFF 改为严格单店：
...
**第 40 轮（OWNER 收紧）：** 用户明确回答"不用"支持 OWNER 多 Merchant。
...
```text
停用不是删除，可通过状态迁移恢复
DISABLED --> NORMAL（User Security State 状态机允许）
```
...
```text
User Security State 的完整合法迁移矩阵（在 08 收口中细化）
Membership 状态完整枚举（ACTIVE / DISABLED / 其他）
Merchant FROZEN 的具体业务含义与恢复流程
