...
```text
OWNER 和 STAFF 都使用 SYSTEM_ADMIN 提供的邀请码注册
```
...
User
  ↓
Membership A → Merchant A
Membership B → Merchant B
```
...
因此之前设计的"A 店店长重置员工密码可能影响 B 店 STAFF 身份"这个场景已经不存在。
...
OWNER 是否未来支持多个 Merchant——已在第 40 轮由用户回答"不用"，同样收紧为单 Merchant。
...
stateDiagram-v2
    [*] --> CREATED
    CREATED --> ACTIVE
    ACTIVE --> BOUND: OWNER 注册并创建 Merchant
    BOUND --> EXHAUSTED: STAFF 配额全部用完
    ACTIVE --> DISABLED
    BOUND --> DISABLED
    ACTIVE --> EXPIRED
    BOUND --> EXPIRED
...
1. OWNER 和 STAFF 都使用 SYSTEM_ADMIN 提供的邀请码注册；
...
是否允许受邀请成为已有 Merchant 的第二个 OWNER（D01-03，当前主规则不允许）
```

> 来源：对话第 30 轮遗留决策清单 D01-01 ~ D01-05（其中 D01-01、D01-02 已冻结为"单 Merchant"）
