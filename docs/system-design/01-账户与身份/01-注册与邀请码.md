...
```text
建议：expires_at = NULL 表示长期有效；有值时到期进入 EXPIRED。
EXPIRED 只禁止新注册，不影响已创建的 User、Merchant、Membership。
建议：ACTIVE Merchant 下最后一个 OWNER 不能直接停用，必须先完成交接，或先冻结 / 关闭 Merchant。
建议：OWNER 可自助关闭 Merchant，但不能物理删除；关闭前需要重新验证身份并二次确认，关闭后保留历史数据并允许查询 / 导出，SYSTEM_ADMIN 可恢复。
```
