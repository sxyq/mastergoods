...
> 首期 Verification Method：EMAIL。
> PHONE Strategy / SMS Adapter 仅保留扩展能力，首期默认 disabled。
...
flowchart TD
    ...
    REG -. future / disabled .-> PHONE[Phone Strategy]
    REG --> EMAIL[Email Strategy（首期）]
    REG -.未来.-> TOTP[TOTP]
    REG -.未来.-> PASSKEY[Passkey]

    PHONE -. future / disabled .-> SMS[SMS Provider Adapter]
    EMAIL --> MAIL[Email Provider Adapter]
```

**图示说明（1. 可扩展验证）：** 这是责任或数据流图，核心节点包括 Registration / Recovery / Re-auth / Change Identifier。箭头表示调用、归属或数据流向；权限、Merchant 范围和状态判断必须在服务端完成，不能由客户端或单一 UI 分支替代。
...
Phone Strategy 不知道 Twilio、阿里云、腾讯云等具体 API；首期不注册该 Strategy。
...
**图示说明（7. Outbox）：** 这是服务调用时序图，参与者包括 Verification Service、PostgreSQL、Outbox Worker、SMS / Email Provider。箭头表示一次调用或返回，alt/else 分支表示 Decision 的不同结果；事务提交前只做校验和状态准备，短信、邮件等外部副作用应在提交后通过 Outbox 执行。
...
1. EMAIL 首期；PHONE Strategy / SMS Adapter 默认 disabled；
2. Strategy 扩展；
