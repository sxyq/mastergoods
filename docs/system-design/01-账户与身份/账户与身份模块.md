...
| Login Identifier | EMAIL（首期）；PHONE 仅保留扩展模型 |
...
**Login Identifier**：首期只使用 EMAIL 登录；一个 User 可以同时保存手机号和邮箱，但 PHONE 首期不用于登录或恢复。

**Password Credential**：正式密码凭据属于 User；数据库不保存明文或可逆"可查看密码"。

**Verification Channel**：表示手机号 / 邮箱是否已验证，以及当前策略是否允许其用于登录、恢复或 Step-up；首期仅启用 Email Challenge。
...
> 本节同时记录已确定规则和建议基线。P-03（单 Merchant 单 OWNER、交接换店长）已确定；会话时长、首期验证通道、密码恢复通道等 P-01、P-02、P-04—P-17 仍以 `待确认事项.md` 的状态为准，未因写入本节而自动冻结。
...
14. 首期验证方式为 EMAIL；PHONE Strategy / SMS Adapter 保留扩展能力但默认 disabled；
15. 验证方式必须可扩展（Strategy + Registry + Adapter）；
16. 用户自己改 / 重置密码支持：当前密码、Email Challenge；PHONE Challenge 保留扩展能力但首期禁用；
