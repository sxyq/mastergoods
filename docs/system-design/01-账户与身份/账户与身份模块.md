...
例如一个 STAFF 同时在两个 Merchant 工作（历史模型，后续已收紧为单店）：
...
A 店店长修改这个员工的店内备注，不应该把 B 店资料一起改掉。
...
**当前产品不支持 OWNER 多 Merchant，也不支持 STAFF 多 Merchant（已冻结）。**
...
flowchart TD
    ...
    D08 --> D4[04 Account / Session State Machine]
