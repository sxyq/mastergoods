...
- **OWNER 与 STAFF 均只拥有一个 Merchant Membership**（对话第 40 轮冻结）。
...
当前 Source of Truth 固定为微信小程序、Android App、iOS App、Web 四端；四端复用同一套 Backend API。
...
**图示说明（3. 技术栈与平台架构）：** 这是责任或数据流图，核心节点包括 微信小程序、Android App、iOS App、Web。箭头表示调用、归属或数据流向；权限、Merchant 范围和状态判断必须在服务端完成，不能由客户端或单一 UI 分支替代。

技术组合建议（对应待确认事项 P-01；确认前作为推荐基线，不标为冻结）：

| 层 | 建议 |
|---|---|
| Backend | **Go；模块化单体（推荐）** |
| 架构边界 | **Modular Monolith**；模块经 Application Service / Domain Service 协作，Repository / Unit of Work 隔离持久化 |
| HTTP | `net/http + chi` |
| 数据访问 | `pgx + sqlc` |
| 主数据库 | **PostgreSQL** |
| Agent | **Go，与 Backend 同代码库** |
| Agent 流式 | **SSE** |
...
待确认状态（详见 [`待确认事项.md`](待确认事项.md) 的 P-01）：

```text
建议确认 Go + PostgreSQL + 模块化单体作为第一阶段技术基线；
四端为微信小程序原生 TS、Android Kotlin + Jetpack Compose、iOS Swift + SwiftUI、Web React + TypeScript + Vite。
```
...
flowchart TD
    ...
    A --> A2[邀请码注册 ✅]
    A --> A3[Email 验证 ✅；PHONE future / disabled]
...
完成 08 六项收口（注册准入 Decision、登录 Decision Pipeline、Password Reset Strategy Resolver、Account / Session State Machine 矩阵、Risk Decision Engine、幂等 / 并发 / Transaction）后，在顶层文档标记：
...
```text
08 已完成最后一轮收口；账户与身份模块停止继续发散，下一步进入卖货模块。
```
...
账户与身份已完成该收口流程；下一轮从卖货模块的二级用例和主时序开始。
