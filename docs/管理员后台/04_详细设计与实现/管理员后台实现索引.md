...
| 独立管理员 Web 发布与基础验收 | 已完成（基础范围） | 已发布至 `/zhj-admin/`；公网入口、SPA 路由、JS/CSS 资源返回 200，登录页可见且浏览器控制台错误为 0；未认证 `/v2/admin/session` 返回 401 |
...
管理员后端源码、V35-V41 迁移已创建并已部署；独立管理员 Web 源码已实现、构建并发布，基础公网/浏览器核对已完成。此前嵌入店主端的管理员页面/路由已由 `fa14a61b` 回退并发布。独立 Web 尚未使用真实登录凭据，真实登录/退出、带权限页面、跨 owner/store 数据范围、SSE 断线重连、PostgreSQL 查询计划、管理员 Web 性能实测和高风险操作仍待执行；Android 真实链路也仍未执行，不能用旧设备证据替代。
