...
| 阶段 | 内容 | 依赖 | 交付物 | 状态 |
|---|---|---|---|---|
| I0 | 现有认证、角色、门店和 Agent 数据核对 | 当前源码 | 字段与调用链确认 | 未完成 |
| I1 | 管理员身份、角色和范围 | I0 | principal、权限字典、范围服务 | 未完成 |
| I2 | 总览、组织和运行只读查询 | I1 | Controller、DTO、分页 Query | 未完成 |
| I3 | Agent 事件、消息、Token 和上下文 | I2 | 观测服务、投影、脱敏 | 未完成 |
| I4 | 管理员审计、配置和导出 | I1–I3 | 事务写服务、任务和审计 | 未完成 |
| I5 | Vue 管理员页面 | I2–I4 | 路由、页面、组件和状态 | 未完成 |
| I6 | 测试、性能验证和发布准备 | I1–I5 | 测试报告、查询计划、交付清单 | 未完成 |
...
| 正式管理员前端目录 | 进行中 | `Code/frontend/web/src/entities/admin/`、`features/admin/`、`pages/admin/` 和管理员 API 契约骨架已创建，页面尚未接入路由 |
| 正式管理员后端目录 | 进行中 | `Code/backend/src/main/java/com/zhihuiji/backend/{api/dto/admin,application/service/admin,infrastructure/security/admin,infrastructure/repository/admin,api/controller/admin}` 骨架已创建，Controller 和查询实现尚未接入 |
| 管理员专用审计存储 | 未完成 | 尚未确定复用/新增方案 |
| Agent actor/store 运行关联 | 未完成 | 当前审计表需进一步核对 |
| 接口和权限契约 | 规划中 | `02_`、`03_` 文档 |

## 对应实现

本文件索引的目标代码位于正式项目 `Code/`，当前不在临时文档目录创建源码。
...
当前尚未开始正式编码；所有目标文件名是设计建议，需在实现前结合现有包结构确认。
