...
| I5 | Vue 管理员页面 | I2–I4 | 独立路由、页面、组件和状态 | 已实现（静态构建与真实环境验收分开记录） |
...
管理员后端源码、V35-V41 迁移已创建并已部署；独立管理员 Web 源码已实现。此前店主端的管理员页面/路由已由 `fa14a61b` 回退并发布，独立 Web 的真实登录/退出、跨范围页面、SSE 重连、查询计划、性能实测和正式发布仍待执行。
