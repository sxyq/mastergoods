...
| I5 | Vue 管理员页面 | I2–I4 | 独立路由、页面、组件和状态 | 未开始（目录骨架已创建） |
...
| 正式管理员前端目录 | 已建立骨架 | `Code/frontend/admin-web/` 已建立页面、模块、共享 API 和测试目录；尚未写入路由、页面或 API 客户端 |
...
管理员后端源码、V35-V41 迁移已创建并已部署；独立管理员 Web 仅有目录骨架。此前店主端的管理员页面/路由已由 `fa14a61b` 回退并发布，真实 Web 登录/退出、跨范围页面、SSE 重连、查询计划、性能实测和正式发布仍待执行。
