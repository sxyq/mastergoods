...
|---|---|
| 文档类型 | 详细设计与实现索引 |
| 当前状态 | 实现索引已更新；独立 Web 已发布并完成基础公网/浏览器核对，业务验收待执行 |
| 正式前端 | `Code/frontend/admin-web/` |
| 正式后端 | `Code/backend/` |
...
| I5 | Vue 管理员页面 | I2–I4 | 独立路由、页面、组件和状态 | 已实现并已发布（构建、静态资源、SPA 路由和基础页面核对通过；业务登录验收待执行） |
| I6 | 测试、性能验证和发布准备 | I1–I5 | 测试报告、查询计划、交付清单 | 部分完成（构建、发布和基础公网/浏览器核对通过；业务、性能和恢复项待执行） |
...
| `Code/frontend/admin-web/src/pages/` | 管理员页面 | `BR-F01`–`16` |
| `Code/frontend/admin-web/src/features/` | 组件、权限、SSE 和状态 | `SR-ADM-02`–`14` |
...
| 正式管理员前端目录 | 已实现 | `Code/frontend/admin-web/` 已建立独立路由、内存会话、共享 API、SSE 客户端、页面、图表和通用状态组件 |
...
| 独立管理员 Web 构建 | 已通过 | `Code/frontend/admin-web/` 执行 `VITE_API_BASE_URL=https://zhj-api.sxyq27.online npm run build`；`vue-tsc` 和 Vite 共转换 1634 个模块 |
| 独立管理员 Web 发布与基础验收 | 已完成（基础范围） | release `76b2a13a` 已发布至 `https://sxyq27.online/zhj-admin/`；公网入口、SPA 路由、JS/CSS 资源返回 200，登录页可见且浏览器控制台错误为 0；未认证 `/v2/admin/session` 返回 401 |
...
管理员后端源码、V35-V42 迁移已创建并已部署；目标运行容器为 `sxyq27-zhj-api:20260901T014000-agent-owner-bill-c012290b`，目标数据库当前为 Flyway V42。独立管理员 Web 源码已实现、构建并发布，基础公网/浏览器核对已完成。此前嵌入店主端的管理员页面/路由已由 `fa14a61b` 回退并发布。真实管理员登录/退出、带权限 API、跨 owner/store HTTP、SSE 断线重连、PostgreSQL 查询计划、管理员 Web 性能实测和高风险操作仍待执行；Android Agent 全链路为 `Blocked`（测试凭据不可用），iOS 本轮为 `Deferred`。
