...
| 独立管理员 Web 发布与基础验收 | 已完成（基础范围） | release `550e19f6` 已发布至 `https://sxyq27.online/zhj-admin/`；公网入口、SPA 路由、JS/CSS 资源返回 200，登录页可见且浏览器控制台错误为 0；未认证 `/v2/admin/session` 返回 401 |
...
管理员后端源码、V35-V41 迁移已创建并已部署；目标运行容器为 `sxyq27-zhj-api:20260831T112015-C-worktree-owner-agent`，目标数据库当前为 Flyway V41。独立管理员 Web 源码已实现、构建并发布，基础公网/浏览器核对已完成。此前嵌入店主端的管理员页面/路由已由 `fa14a61b` 回退并发布。真实管理员登录/退出、带权限 API、跨 owner/store HTTP、SSE 断线重连、PostgreSQL 查询计划、管理员 Web 性能实测和高风险操作仍待执行；Android Agent 全链路为 `Blocked`（测试凭据不可用），iOS 本轮为 `Deferred`。
