...
F1 至 F7 已完成源码实现：独立路由、统一登录、内存 token 续期、侧栏、总览、用户与门店成员、Agent 运行与 SSE、配置、审计、导出和系统保留策略页面均位于 `Code/frontend/admin-web/src/`。门店 Owner 当前只读，转移接口未实现。F8 的静态构建已通过 `vue-tsc -b && vite build`；组件、真实浏览器、真实 API、权限菜单和 SSE 重连仍按环境验收计划执行。
...
前端测试计划位于 `../05_测试与验收/功能测试/管理员后台功能测试计划.md`；当前未发布独立管理员 Web，浏览器交互、真实 API 映射、权限菜单和 SSE 重连仍待执行。

## 当前限制

临时原型是 React。此前页面落入店主端的实现已由 `fa14a61b` 保存并回退，店主端回退版已发布；新工程计划使用独立入口 `https://sxyq27.online/zhj-admin/`，尚未发布。独立 Web 已接入统一登录、退出和内存 token 续期；整页刷新会回到登录页。真实 API 数据映射、权限菜单、SSE 重连、组件和浏览器验收仍待执行。
