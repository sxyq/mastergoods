...
| F8 | 视觉、可访问性和构建 | Web build 已通过；组件、键盘和浏览器验收待执行 |
...
| 检查 | 命令/证据 | 结果要求 |
|---|---|---|
| 类型与构建 | `PATH=/Users/sunyiyang/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH npm run build` | 成功（2026-08-30） |
| 组件状态 | 组件测试 | 成功/明确阻塞 |
...
- 管理员路由：`Code/frontend/web/src/app/router/admin-routes.ts`、`app/router/routes.ts`
- 管理员 session/API：`Code/frontend/web/src/app/stores/admin-session.ts`、`shared/api/admin.ts`
- 管理员页面：`Code/frontend/web/src/pages/admin/`
- 管理员组件/样式/SSE：`Code/frontend/web/src/features/admin/`、`shared/api/agent-stream.ts`
...
前端测试计划位于 `../05_测试与验收/功能测试/管理员后台功能测试计划.md`；本轮已完成 Web build 和目标站点发布，浏览器交互、真实 API 映射、权限菜单和 SSE 重连仍待执行。

## 当前限制

临时原型是 React，正式页面已落在现有 Vue Web 工程并注册 `/admin/*` 路由。本轮 Web build 已通过并发布到 `https://sxyq27.online/zhj/`；真实 API 数据映射、登录/退出、权限菜单、SSE 重连、组件和浏览器验收仍待执行。
