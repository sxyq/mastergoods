...
临时原型是 React。此前页面落入店主端的实现已由 `fa14a61b` 保存并回退，店主端回退版已发布；新工程计划使用独立入口 `https://sxyq27.online/zhj-admin/`，尚未发布。真实 API 数据映射、登录/退出、权限菜单、SSE 重连、组件和浏览器验收仍待执行。
