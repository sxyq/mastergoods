...
|---|---|
| 文档类型 | 数据库与读模型设计 |
| 当前状态 | 迁移与模型已实现；目标版本和汇总数据已核对，数据覆盖与查询计划待验证 |
| 数据源 | 现有共享数据库；当前源码和目标环境迁移版本均已核对至 V42 |
...
- 本轮已直接核对目标 PostgreSQL 的 Flyway V42 和脱敏汇总计数；未执行新的数据库迁移、不改变正式数据、不创建测试数据。新鲜库迁移、带夹具迁移、查询计划和跨范围 HTTP 仍需单独验证。

当前迁移实现：V35 建立管理员账户/范围，V36 增加 Agent 运行 actor/store，V37 建立管理员审计、Agent 配置、导出任务和保留策略，V38 增加用户/门店/成员管理版本，V39 保存导出创建时的范围与时间快照，V40 保存草稿确认人、确认时间、业务引用和失败原因，V41 允许匿名拒绝审计省略管理员 ID，V42 建立 owner 范围单据编号唯一索引。
...
- 迁移：`Code/backend/src/main/resources/db/migration/V35__administrator_identity_and_scope.sql` 至 `V42__owner_scoped_bill_numbers.sql`
...
管理员测试已覆盖实体/服务契约、范围、分页、审计、导出、配置、V40/V41/V42 SQL 文本和 V41 匿名拒绝约束；目标 PostgreSQL 当前为 V42，真实查询计划、历史数据覆盖、跨范围 HTTP 和生产保留策略仍需验证。

## 当前限制

管理员审计、范围授权、模型配置、导出任务、保留策略和管理版本对象已存在；V36/V40/V41 字段对历史数据保持可为空。目标 PostgreSQL V42 已核对，真实新鲜库/已有数据迁移复验、查询计划、跨 owner/store HTTP 和生产保留策略仍待验证。
