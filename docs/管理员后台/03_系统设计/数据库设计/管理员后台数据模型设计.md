...
管理员登录仍复用统一认证上下文，当前没有独立的 `admin_login_events` 表；session 查询和管理员操作由 `admin_audit_events` 记录。任何后续结构变更必须包含必要的 owner/store 约束、索引、保留策略和迁移测试。
...
当前迁移实现：V35 建立管理员账户/范围，V36 增加 Agent 运行 actor/store，V37 建立管理员审计、Agent 配置、导出任务和保留策略，V38 增加用户/门店/成员管理版本，V39 保存导出创建时的范围与时间快照，V40 保存草稿确认人、确认时间、业务引用和失败原因。
...
管理员测试已覆盖实体/服务契约、范围、分页、审计、导出、配置和 V40 SQL 文本约束；仍需在真实 PostgreSQL 执行迁移、查询计划、历史数据和跨范围 HTTP 验证。
