...
| 数据源 | 现有共享数据库；当前源码和目标环境迁移版本均已核对至 V41 |
...
- 本轮已直接核对目标 PostgreSQL 的 Flyway V41 和脱敏汇总计数；未执行新的数据库迁移、不改变正式数据、不创建测试数据。新鲜库迁移、带夹具迁移、查询计划和跨范围 HTTP 仍需单独验证。
...
管理员测试已覆盖实体/服务契约、范围、分页、审计、导出、配置、V40/V41 SQL 文本和 V41 匿名拒绝约束；目标 PostgreSQL 当前为 V41，真实查询计划、历史数据覆盖、跨范围 HTTP 和生产保留策略仍需验证。

## 当前限制

管理员审计、范围授权、模型配置、导出任务、保留策略和管理版本对象已存在；V36/V40/V41 字段对历史数据保持可为空。目标 PostgreSQL V41 已核对，真实新鲜库/已有数据迁移复验、查询计划、跨 owner/store HTTP 和生产保留策略仍待验证。
