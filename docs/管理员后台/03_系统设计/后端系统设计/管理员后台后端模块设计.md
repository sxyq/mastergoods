...
│   ├── AdminAgentEventStreamService.java
│   ├── AdminAgentRuntimeConfigService.java
│   └── AdminExportWorker.java
├── infrastructure/repository/admin/
│   ├── AdminUserQueryRepository.java
│   ├── AdminStoreQueryRepository.java
│   ├── AdminAgentQueryRepository.java
│   ├── AdminAgentDetailQueryRepository.java
│   ├── AdminAgentEventQueryRepository.java
│   ├── AdminAuditEventRepository.java
│   └── AdminExportJobRepository.java
...
目录已按正式工程现有包结构落地；实现继续复用 `ApiResponse`、`GlobalExceptionHandler`、`PaginationUtils` 和现有 owner/store 关系组件。

## 二、服务职责

| 服务 | 输入 | 处理 | 输出 |
|---|---|---|---|
| `AdminPrincipalResolver` | 当前统一会话 | 解析管理员身份、角色、范围和会话状态 | `AdminPrincipal` |
...
| `AdminSystemService` | 模型 ID、工具开关、版本、原因 | 校验、事务写入、刷新配置、健康状态 | 配置版本/健康 DTO |
| `AdminAuditService` | 操作上下文和结果 | 写入管理员审计，连接 Agent 审计 ID | 审计事件 |
| `AdminAgentDetailService` | DTO 和内容模式 | 事件解析、字段白名单、长度限制、敏感内容处理 | 可展示摘要 |
...
V36 已为 `agent_run_audits` 增加可为空的 `actor_user_id`、`store_id` 及索引；历史运行没有可靠来源时仍返回 `null`，服务不会根据 owner 或会话名称推断。当前真实 Agent 运行写入链和历史数据覆盖需要在环境中继续核对。
...
管理员 Controller、DTO、查询 Repository、认证解析、审计、配置、导出和保留策略实现已存在，管理员契约测试已通过。当前复用统一认证，管理员 Web 使用既有认证接口建立和续期会话；目标环境后端已完成基础发布，运行容器为 `sxyq27-zhj-api:20260901T014000-agent-owner-bill-c012290b`，目标数据库当前为 Flyway V42。真实管理员带权限 API、跨范围数据集、审计失败处置、Provider 用量语义、SSE 重连和 PostgreSQL 查询计划仍未验证。
