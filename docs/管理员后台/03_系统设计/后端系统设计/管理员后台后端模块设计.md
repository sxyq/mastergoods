...
管理员 Controller、DTO、查询 Repository、认证解析、审计、配置、导出和保留策略实现已存在，管理员契约测试已通过。当前复用统一认证，管理员 Web 使用既有认证接口建立和续期会话；真实 HTTP、跨范围数据集、审计失败处置、Provider Token 累计语义和生产部署仍未验证。
