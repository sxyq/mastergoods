...
- 正式 Web 会话：`Code/frontend/web/src/app/stores/session.ts`
- 正式 Web API 客户端：`Code/frontend/web/src/shared/api/client.ts`
- 正式 Agent 页面：`Code/frontend/web/src/pages/agent/AgentPage.vue`
...
使用 Vue 组件测试、API mock 契约测试、权限路由测试、事件序列测试和 Web build；浏览器真实登录和点击测试需要在另行授权的环境中执行，当前原型 QA 不代表正式后台通过。

## 当前限制

正式 Web 工程已创建管理员实体契约、API 路径、页面和功能模块骨架；临时原型使用 React，不能直接当作正式 Vue 实现。管理员 session 确认前，页面路由仍不接入普通门店导航。
