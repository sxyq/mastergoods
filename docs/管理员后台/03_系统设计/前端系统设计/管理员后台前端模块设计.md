...
|---|---|
| 文档类型 | 前端系统设计 |
| 当前状态 | 独立工程已完成构建并发布；入口、SPA 路由、资源和浏览器基础状态已核对；真实登录与业务数据验收待执行 |
...
当前已实现 `/login`、`/overview`、`/users`、`/agent/runs`、`/agent/config`、`/audit`、`/system` 和 `/forbidden`。页面使用 `AdminLayout.vue`、`admin-session.ts`、`shared/api/` 与 `pages/` 中的独立实现，不引用店主端路由或 session。

用户与门店页将 Owner 显示为只读归属信息。当前后端没有 Owner 转移命令，超级管理员可编辑的门店字段限于名称和状态；页面不构造无效的 owner 变更请求。
...
截至 2026-08-31，独立管理员 Web 已完成构建与发布基础核对：`npm run build` 通过；`/zhj-admin/`、`/zhj-admin/agent/runs`、JS 和 CSS 资源均返回 HTTP 200；Nginx 配置检查和 reload 通过；登录页可见、自动填充内容已清空、浏览器控制台错误为 0。未鉴权请求 `GET /v2/admin/session` 返回 HTTP 401，这只证明未登录访问被拒绝，不能推断真实管理员登录成功。

以下事项仍未完成，不能据此宣告前端正式验收通过：真实管理员登录和退出、带权限的真实 API 数据页、跨 owner/store 范围、响应式与可访问性深入验收、真实管理员 SSE 连接/断线重连，以及 Android Agent 联动验收（`Blocked`，测试凭据不可用）。iOS 本轮为 `Deferred`。
...
店主端内的管理员页面属于历史错误实现，已由 Git 保留后回退。独立工程已具备构建配置和页面源码；正式验收仍需补充 Vue 组件测试、API mock 契约测试、权限路由测试、事件序列测试和真实浏览器流程。

## 当前限制

临时原型使用 React，只作为视觉核验依据。独立管理员工程已使用统一认证的登录、退出和内存 token 续期；整页刷新会清空页面内凭据并回到登录页。当前已确认独立管理员 Web 的构建、发布、入口、SPA 路由、JS/CSS 资源和登录页基础浏览器状态；未鉴权 `GET /v2/admin/session` 返回 401，仅作为拒绝未登录请求的证据。真实管理员登录、退出、带权限的真实 API 数据页、跨 owner/store、响应式与可访问性深入验收、管理员 SSE 以及 Android Agent 联动仍待执行（`Blocked`，测试凭据不可用）；iOS 本轮为 `Deferred`。
