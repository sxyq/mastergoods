...
管理员模块已在正式 `Code/` 目录接入 Controller、查询 Service/Repository、服务端身份解析、管理员页面路由和 V35-V40 数据结构。当前复用统一认证，只有 session 查询而无独立登录/退出；历史运行的 actor/store 可为空，真实运行写入链、跨范围 HTTP、SSE 重连、PostgreSQL 计划和生产部署仍待验证。
