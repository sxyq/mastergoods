...
上述管理员目录已在正式工程创建并接入路由/Controller；本临时目录只保留视觉原型和规划文档，不承担正式运行链路。
...
| 构建 | 临时 Vite 构建已核验 | 正式 Web build 已通过；接口与真实环境测试待执行 |

## 对应实现

- 正式 Web 管理员模块：`Code/frontend/web/src/pages/admin/`、`features/admin/`、`app/router/admin-routes.ts`
...
管理员源码/契约测试已执行并通过：`./Code/backend/gradlew -p Code/backend test --tests '*admin.*' --no-parallel --rerun-tasks --no-daemon`（2026-08-30）。Web build、真实 HTTP、跨范围数据集、SSE 重连、PostgreSQL 计划和生产部署仍需单独验收。

## 当前限制

管理员模块已在正式 `Code/` 目录接入 Controller、查询 Service/Repository、服务端身份解析、管理员页面路由和 V35-V41 数据结构。当前复用统一认证，只有 session 查询而无独立登录/退出；历史运行的 actor/store 可为空，真实运行写入链、跨范围 HTTP、SSE 重连、PostgreSQL 计划和本轮 V41 发布仍待验证。
