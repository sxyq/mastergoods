...
| 正式前端基线 | `Code/frontend/admin-web/`，计划使用 Vue 3 + TypeScript + Vite |
...
后端目录已接入 Controller；独立管理员前端当前只建立目录骨架，尚未接入路由或页面。本临时目录只保留视觉原型和规划文档，不承担正式运行链路。

## 六、部署与网络边界

- 管理员 Web 使用独立工程和独立构建产物，计划发布到 `/zhj-admin/`；店主端继续使用 `/zhj/`。
...
管理员后端接口已完成真实 HTTP 功能验证。独立管理员 Web 的构建、浏览器流程、跨范围数据集、SSE 重连和性能验收仍需单独执行。

## 当前限制

管理员后端已在正式 `Code/` 目录接入 Controller、查询 Service/Repository、服务端身份解析和 V35-V41 数据结构。此前嵌入店主端的管理员页面已被认定为架构错误，已由 `fa14a61b` 回退；当前复用统一认证，独立管理员 Web 的登录入口、跨范围页面、SSE 重连和性能验收待实现。
