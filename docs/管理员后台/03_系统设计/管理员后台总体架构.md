...
管理员后端已在正式 `Code/` 目录接入 Controller、查询 Service/Repository、服务端身份解析和 V35-V41 数据结构。此前嵌入店主端的管理员页面已被认定为架构错误，已由 `fa14a61b` 回退；当前复用统一认证，独立管理员 Web 已实现并发布到 `https://sxyq27.online/zhj-admin/`。真实登录、带权限数据页、跨 owner/store HTTP、SSE 重连和性能验收待目标环境验证。
