...
管理员后端已在正式 `Code/` 目录接入 Controller、查询 Service/Repository、服务端身份解析和 V35-V41 数据结构。此前嵌入店主端的管理员页面已被认定为架构错误，Git 保留后回退；当前复用统一认证，独立管理员 Web 的登录入口、跨范围页面、SSE 重连和性能验收待实现。
