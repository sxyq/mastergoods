...
| 当前状态 | 需求已核对；实现部分完成 |
| 适用端 | 管理员 Web 后台 |
| 依据正式项目 | `Code/backend/`、`Code/frontend/admin-web/`、Flyway V1–V42 |
| 依据原型 | `Temp/grok-style-admin-preview/src/App.jsx` |
| 依据参考源码 | 外部参考源码（未纳入本仓库），版本 `62d2775` |
| 最后核对 | 2026-09-01 |
...
- 管理员服务、权限与查询：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/admin/`、`infrastructure/security/admin/`、`infrastructure/repository/admin/`
- 管理员 DTO 与数据实体：`Code/backend/src/main/java/com/zhihuiji/backend/api/dto/admin/`、`domain/entity/Admin*Entity.java`
- 独立 Web 管理员工程：`Code/frontend/admin-web/`
- Agent 运行写入链：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/v2/agent/`

## 对应接口

当前实现使用受保护的 `/v2/admin/*` 接口空间，具体契约和实际 Controller 映射见 `docs/管理员后台/02_业务系统需求/管理员后台接口与数据需求.md` 与 `docs/管理员后台/03_系统设计/接口设计/管理员后台接口契约设计.md`。服务继续使用统一响应、异常处理、owner/store 条件和分页规范。

## 对应测试

- 原型构建与静态交互：`Temp/grok-style-admin-preview/design-qa.md`
- 管理员后端契约测试：`Code/backend/src/test/java/com/zhihuiji/backend/api/controller/admin/`、`application/service/admin/`、`infrastructure/security/admin/`
- 已执行命令：`./Code/backend/gradlew -p Code/backend test --tests '*admin.*' --no-parallel --rerun-tasks --no-daemon`（2026-08-30，`BUILD SUCCESSFUL`）
- 正式后台环境测试计划：`docs/管理员后台/05_测试与验收/`
- Agent 运行链路：正式项目 `testing/Agent/` 下现有分类计划

## 当前限制

- 当前复用统一认证上下文，已提供 `GET /v2/admin/session`；独立管理员 Web 已通过统一认证 API 实现登录入口与管理员 session 读取，真实环境流程待验证。
- Controller、Service、Repository、DTO、权限解析、管理员审计、配置、导出、保留策略和 Agent 观测后端已上线并完成源码/契约验证；目标后端容器为 `sxyq27-zhj-api:20260901T014000-agent-owner-bill-c012290b`，目标数据库当前为 Flyway V42。真实管理员带权限 API、跨 owner/store HTTP、SSE 重连、PostgreSQL 查询计划和目标环境数据验收仍待验证。
- 临时原型的数字、用户、门店、模型和日志内容仍为脱敏演示数据；独立 Web 已发布到 `https://sxyq27.online/zhj-admin/`，跨 owner/store 数据页、SSE 重连、性能和恢复演练待验证；Android Agent 全链路为 `Blocked`（测试凭据不可用），iOS 本轮为 `Deferred`。
