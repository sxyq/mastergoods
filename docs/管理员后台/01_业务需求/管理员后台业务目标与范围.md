...
| 当前状态 | 需求已核对；实现部分完成 |
| 适用端 | 管理员 Web 后台 |
| 依据正式项目 | `Code/backend/`、`Code/frontend/admin-web/`、Flyway V1–V41 |
...
- Controller、Service、Repository、DTO、权限解析、管理员审计、配置、导出、保留策略和 Agent 观测后端已上线并完成源码/契约验证；目标后端容器为 `sxyq27-zhj-api:20260831T112015-C-worktree-owner-agent`，目标数据库当前为 Flyway V41。真实管理员带权限 API、跨 owner/store HTTP、SSE 重连、PostgreSQL 查询计划和目标环境数据验收仍待验证。
