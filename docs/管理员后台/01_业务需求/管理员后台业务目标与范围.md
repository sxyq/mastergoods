...
| 当前状态 | 需求已核对；实现部分完成 |
| 适用端 | 管理员 Web 后台 |
| 依据正式项目 | `Code/backend/`、`Code/frontend/web/`、Flyway V1–V40 |
...
- 正式 Web 管理员页面与路由：`Code/frontend/web/src/pages/admin/`、`Code/frontend/web/src/features/admin/`、`Code/frontend/web/src/app/router/admin-routes.ts`
...
- 当前复用统一认证上下文，已提供 `GET /v2/admin/session`；尚无独立管理员登录、退出接口，也未做真实登录/退出 HTTP 验证。
- Controller、Service、Repository、DTO、权限解析、管理员审计、配置、导出、保留策略和 Agent 观测代码已存在；当前证据为源码与管理员测试，未启动前后端服务。
- 临时原型的数字、用户、门店、模型和日志内容仍为脱敏演示数据；真实跨 owner/store HTTP、SSE 重连、PostgreSQL 查询计划、生产部署和恢复演练待验证。
