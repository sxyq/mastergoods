...
- 管理员角色与权限：`Code/backend/src/main/java/com/zhihuiji/backend/infrastructure/security/admin/AdminPrincipal.java`、`AdminPermission.java`
- 管理员身份与范围解析：`Code/backend/src/main/java/com/zhihuiji/backend/infrastructure/security/admin/AdminPrincipalResolver.java`、`AdminDataScope.java`
- 服务端授权：`Code/backend/src/main/java/com/zhihuiji/backend/infrastructure/security/admin/AdminAuthorizationService.java`、`Code/backend/src/main/java/com/zhihuiji/backend/application/service/admin/DefaultAdminScopeService.java`
- 持久化授权表：`Code/backend/src/main/resources/db/migration/V35__administrator_identity_and_scope.sql`
- 独立 Web 管理员 session 与菜单：`Code/frontend/admin-web/src/app/stores/`、`Code/frontend/admin-web/src/features/`
...
- 已有 `AdminPrincipalTest`、`AdminRolePermissionContractTest`、`AdminAuthorizationServiceTest`、`DefaultAdminScopeServiceTest`、`AdminAuthorizationBoundaryTest` 和 `AdminPrincipalResolverTest` 覆盖源码级权限与范围边界。
- 每个权限代码仍需要真实 HTTP、跨 owner、跨 store 和无会话环境用例。
...
- 管理员专用角色、权限字典、`admin_accounts` 和 `admin_scope_grants` 已实现；观察员范围由服务端授权表解析，超级管理员使用全 owner 范围。
- 当前复用统一认证上下文。管理员 Web 使用 `/v2/auth/login`、`/v2/auth/refresh`、`/v2/auth/logout` 后读取管理员 session，token 只留在页面内存；真实账号授予、跨范围 HTTP、最后一个超级管理员保护和生产会话失效策略仍待验证或确认。
