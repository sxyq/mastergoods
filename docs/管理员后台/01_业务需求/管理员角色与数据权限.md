...
- Web 管理员 session 与菜单：`Code/frontend/web/src/app/stores/admin-session.ts`、`Code/frontend/web/src/features/admin/components/AdminSidebar.vue`
...
- 当前复用统一认证上下文，尚无独立管理员登录/退出接口；真实账号授予、跨范围 HTTP、最后一个超级管理员保护和生产会话失效策略仍待验证或确认。
