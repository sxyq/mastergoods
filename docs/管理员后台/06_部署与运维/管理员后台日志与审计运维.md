...
- 管理员审计：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/admin/AdminAuditService.java`、`Code/backend/src/main/java/com/zhihuiji/backend/domain/entity/AdminAuditEventEntity.java`
- Agent 审计：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/v2/agent/component/RunAuditService.java`
- 导出/保留策略：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/admin/AdminExportService.java`、`AdminExportWorker.java`、`AdminRetentionService.java`
...
管理员审计存储、导出任务和保留策略代码已存在，相关迁移为 V37、V39；实际保留天数由策略记录和正式合规要求决定，受信代理 IP 解析仍未实现。当前不执行日志清理，真实审计失败、文件过期清理和生产保留周期待验证。
