...
管理员 Controller、身份解析、范围授权、读写服务、审计/配置/导出/保留策略和 Agent 观测实现已存在，V35-V40 迁移已创建。当前没有真实管理员登录/退出、跨 owner/store HTTP、PostgreSQL `EXPLAIN`、目标并发和生产部署证据；上述延迟和并发数值仍是设计目标。
