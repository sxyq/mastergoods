...
- 管理员观测与 SSE：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/admin/AdminAgentDetailService.java`、`AdminAgentEventStreamService.java`
- 原始 Agent SSE：`Code/backend/src/main/java/com/zhihuiji/backend/application/service/v2/agent/component/SseStreamEmitter.java`
...
已有 `AdminAgentSseContractTest`、`AdminAgentEventStreamServiceContractTest` 和相关 Agent 草稿测试覆盖源码级事件、终态和草稿证据；真实跨层证据仍建议目录为 `testing/.artifacts/<日期>-admin-reliability/`。

## 当前限制

源码级 SSE/观测契约测试已有通过证据；目标环境基础发布和 Flyway V41 已核对，本轮未执行故障注入。真实 SSE 断线补读、重复事件、数据库前后变化、Provider 用量、跨 owner/store HTTP 和草稿确认链尚未执行。
