...
flowchart TD
    ...
    J --> K[手机/客户端覆盖式确认]
    K -->|确认| L[正式动作或 Provider]
    K -->|拒绝| M[cancelled，不写正式表]
    I --> N[ToolAudit 记录 returned_count/evidence]
    N --> O[迭代判断 planNextIteration]
    O -->|需要补充| B
    O -->|完成| P[selectVisibleResultBlocks]
```

图表目的：展示工具选择与执行的完整链路。

图中输入：用户消息。
图中处理：`ToolPlanner` 规划 → `ToolRegistry` 校验执行 → CREATE_ONLY 草稿 → 手机/客户端覆盖式确认 → 正式动作或取消 → 审计累积 → ReAct 迭代（最多 3 轮）。
