...
| 依据源码 | `application/service/v2/agent/AgentDraftConfirmService.java`、`application/service/v2/V2AgentConversationService.java`（drafts CRUD）、`api/controller/v2/V2AgentController.java`（drafts 接口）、`api/dto/v2/agent/V2AgentDtos.java`（AgentDraftResponse） |
| 依据测试 | `AgentDraftConfirmServiceTest.java`、`testing/.artifacts/2026-07-28-draft-closedloop/evidence-summary.md` |
...
stateDiagram-v2
    ...
    active --> confirmed: 用户确认 confirmDraft
...
图中处理：`AgentDraftConfirmService.confirmDraft()` 执行写操作；`cancelDraft()` 取消。
图中输出：草稿终态。
...
- 功能测试：`testing/Agent/Agent综合功能与性能测试方案.md`（draft）
