...
| 当前状态 | 已完成 |
| 适用端 | 后端 |
| 依据源码 | `Code/backend/src/main/resources/db/migration/`（V1-V42）、`domain/entity/` |
...
图表目的：展示主要实体关系（基于 Flyway V1-V42 建表与 owner 字段）。
...
5. **管理员后台**：V35-V42（账号/范围、运行范围、审计配置、导出保留、版本字段、草稿确认证据和 owner 范围单据编号）。
...
| V41 | admin_anonymous_denial_audit |
| V42 | owner_scoped_bill_numbers |
