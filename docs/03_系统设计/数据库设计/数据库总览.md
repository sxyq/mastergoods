...
| 当前状态 | 已完成 |
| 适用端 | 后端 |
| 依据源码 | `Code/backend/src/main/resources/db/migration/`（V1-V29）、`domain/entity/` |
| 依据测试 | `ZhihuijiBackendApplicationTests.java` |
| 依据证据 | `testing/.artifacts/2026-08-18-8220-current-baseline/current-8220-baseline.md`（PostgreSQL 15、Flyway V29） |
| 最后核对 | 2026-08-20 |
...
图表目的：展示主要实体关系（基于 Flyway V1-V29 建表与 owner 字段）。
...
4. **Agent 扩展**：V13（media+agent）、V15（run audits）、V16（消息结构化数据 text 列）、V17（审计丢损指标）、V29（消息 run_id）。
5. **同步**：V26/V27/V28（sync_operation_log、change_log scope、tombstone store key）。
