...
| 3 | `master-goods-android/feature/agent/src/main/java/com/zhihuiji/feature/agent/AgentChatScreen.kt:881` | `pass` | 输入框 placeholder 文案，提示用户输入经营问题；不生成 placeholder 数据或默认报表结果。 |
...
| 13 | `master-goods-android/feature/agent/src/main/java/com/zhihuiji/feature/agent/ResultBlockRenderer.kt:1112` | `pass` | 负向防护文案：当图表缺少真实标签时停止绘制，明确避免生成模拟标签。 |
| 14 | `master-goods-android/feature/agent/src/main/java/com/zhihuiji/feature/agent/ResultBlockRenderer.kt:1115` | `pass` | 负向防护文案：当图表缺少真实标签时停止绘制，明确避免生成模拟标签。 |
...
| 19 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:1812` | `pass` | 日志、错误摘要、图表标签、conversation title 或 UI 摘要的长度裁剪；不用于本地打字机、规则摘要分块或 fake model_stream。 |
...
| 21 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:1886` | `pass` | 负向安全/诚实文案，要求工具失败时不得用模拟数据替代真实查询。 |
| 22 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:2481` | `pass` | 日志、错误摘要、图表标签、conversation title 或 UI 摘要的长度裁剪；不用于本地打字机、规则摘要分块或 fake model_stream。 |
| 23 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:2543` | `pass` | 日志、错误摘要、图表标签、conversation title 或 UI 摘要的长度裁剪；不用于本地打字机、规则摘要分块或 fake model_stream。 |
| 24 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:2646` | `pass` | 日志、错误摘要、图表标签、conversation title 或 UI 摘要的长度裁剪；不用于本地打字机、规则摘要分块或 fake model_stream。 |
| 25 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:2709` | `pass` | 日志、错误摘要、图表标签、conversation title 或 UI 摘要的长度裁剪；不用于本地打字机、规则摘要分块或 fake model_stream。 |
| 26 | `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:2724` | `pass` | 日志、错误摘要、图表标签、conversation title 或 UI 摘要的长度裁剪；不用于本地打字机、规则摘要分块或 fake model_stream。 |
