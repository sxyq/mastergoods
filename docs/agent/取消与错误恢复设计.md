...
| 依据源码 | `application/service/v2/V2AgentAiService.java`（`cancelRun`、`runChatStream` 异常路径）、`application/service/v2/AgentImageService.java`（生图超时/取消）、`application/service/v2/agent/component/RunAuditService.java`（ActiveAgentRun）、`SseStreamEmitter.java`（emitRunCancelled） |
...
生图 Provider 调用只发生在 `image_generate` 草稿确认后。连接超时为 10000ms，读取超时为 120000ms；取消或超时不得被写成成功，Provider 连接和临时资源必须释放，草稿保持可重试并保留脱敏审计。工具创建阶段、拒绝和确认前取消的 Provider 调用次数必须为 0。
...
- 功能测试：`testing/Agent/功能/TEST_PLAN.md`、`testing/Agent/可靠性/TEST_PLAN.md`（取消与故障）
...
- Deferred 内容：生图 Provider 取消传播、资源释放和客户端状态展示
