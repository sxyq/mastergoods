...
| 依据测试 | `V2AgentAiServiceTest.java`、`testing/Agent/README.md`、`testing/Agent/映射台账.md` |
...
`image_generate` 的工具事件记录工具名、`tool_call_id`、草稿 ID、状态、耗时和错误码；`tool_completed` 只代表 active 草稿创建。确认后的 Provider 成功、失败、超时、取消、结果类型（URL 或 b64）和资源清理状态要能按草稿/运行 ID 对齐。审计摘要、SSE 输入摘要和通用日志不得保存 API key、认证头、完整提示词、完整 `b64_json` 或未经脱敏的结果 URL。
...
- 审计：`testing/Agent/README.md` 规定的各类别 `artifacts/`、`logs/`、`reports/`
...
- Deferred 内容：生图 Provider 结果审计、URL/b64_json 脱敏和资源清理证据
