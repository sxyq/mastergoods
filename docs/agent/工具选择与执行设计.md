...
| 依据测试 | `ToolPlannerTest.java`、`ToolRegistryTest.java`、`ToolInvocationIdentityTest.java`、`ImageGenerateToolTest.java`、各 `*LookupToolTest.java` |
...
当前注册目录为 61 个工具：46 个 `READ_ONLY` 与 15 个 `CREATE_ONLY`。全量名称及实现文件以 [代码事实基线](../../../testing/Agent/代码事实基线.md) 第二节为准；本节按领域展示，不能用分组行数替代源码计数。
...
`ImageGenerateTool` 是新增的 `CREATE_ONLY` 工具，正式名为 `image_generate`，权限为 `agent:write`。Schema 为必填 `prompt`（字符串，1 至 2000 个字符）和可选 `reference_asset_ids`（正整数数组，最多 1 个），拒绝未知字段。它的 `execute` 只保存 owner-scoped 的 active 草稿并返回生图 `draft_card`；Provider 调用发生在客户端覆盖式确认后的 `AgentDraftConfirmService.dispatchCreate`。
...
flowchart TD
    ...
    C -->|否| D[直接文本回答]
    C -->|是| E[AgentToolPlan]
    E --> F[ToolExecutor：范围/Schema/权限/上下文]
    F -->|失败| G[tool_failed / 跳过]
    F -->|通过| H{工具类型}
    H -->|READ_ONLY| I[直接执行]
    H -->|CREATE_ONLY| J[ImageGenerateTool 或其他创建工具]
    J --> K[agent_drafts active / draft_created]
    K --> L[手机/客户端覆盖式确认]
    L -->|确认| M[AgentDraftConfirmService]
    M --> N[正式动作或 AgentImageService → Provider]
    L -->|拒绝| O[cancelled，不写正式表]
    I --> P[ToolAudit 记录 returned_count/evidence]
    N --> P
    P --> Q[迭代判断 planNextIteration]
    Q -->|需要补充| B
    Q -->|完成| R[selectVisibleResultBlocks]
```

图表目的：展示工具选择与执行的完整链路。

图中输入：用户消息。
图中处理：`ToolPlanner` 规划 → `ToolRegistry` 查找 → `ToolExecutor` 执行门 → `ImageGenerateTool` 或其他 CREATE_ONLY 工具生成草稿 → 手机/客户端覆盖式确认 → `AgentDraftConfirmService` → 正式动作或 `AgentImageService`/Provider → 审计累积 → ReAct 迭代（最多 3 轮）。
图中输出：工具结果、结果块、草稿、正式生图响应、审计事件。

对应源码：`ToolPlanner.java`、`ToolRegistry.java`、`V2AgentAiService.buildResponse()`。
对应接口：Agent 链路；独立 REST 生图为 `POST /v2/agent/images/generate`，不计为 AgentTool 调用。
对应测试：`ToolPlannerTest.java`、`ToolRegistryTest.java`、`ToolInvocationIdentityTest.java`、`ImageGenerateToolTest.java`。
当前状态：已完成。
...
- 生图 Provider：连接超时 10000ms、读取超时 120000ms；配置键为 `agent.image.base-url`、`agent.image.api-key`、`agent.image.model`，值只使用运行时占位符。
...
- `image_generate` 的 `tool_completed` 只表示草稿已创建；确认后的 Provider 成功/失败、超时或取消必须在草稿响应和审计中区分，不能把 Provider 结果伪装成工具阶段结果。
- Provider `url` 与 `b64_json` 只允许进入授权结果响应；审计和日志保存类型、大小、状态及脱敏摘要，不保存完整二进制或密钥。
...
- 审计：`testing/Agent/README.md` 规定的分类证据目录

## 当前限制

- 未完成内容：无
- Blocked 内容：8220 无业务数据，真实工具查询 Blocked
- Deferred 内容：`image_generate` 的真实 Provider 结果、客户端确认展示和结果 URL/b64_json 安全验证
