...
| B07 媒体与 AI 扩域 | 新版已做 | media/agent 后端合同已落地（media_assets/bindings、agent_conversations/messages/drafts），已补会话更新/删除、状态约束与级联删除；2026-06-30 补拿到媒体上传/绑定/内容读取真闭环；2026-07-03 本地已补真实 provider `model_stream` SSE + audit 证据，但 Android 配对与发布级验收仍归 B11 |
...
| B11 联调、测试、性能、安全、发布验收 | 待验证 | 验收矩阵、证据目录约定与本地复验脚本入口已建立；本轮（2026-07-03）已补齐计划阶段 2-5 全部代码项（P0 临时.md 10 项硬反例、P1 B08 阻断 core/database v2 + data 切 v2、P2 B09 feature 切 v2、P3 B10 UI 母版迁移），后端 `./gradlew clean test --rerun-tasks` 真实重跑 `BUILD SUCCESSFUL in 22s`（7 executed，非 UP-TO-DATE，证据 `docs/acceptance-evidence/b11/backend/20260703-0935-backend-smoke-rerun.log`）；Android/Web 本地构建在当前工作树上仍通过；2026-07-03 又补到本地真实 provider `model_stream` SSE + audit 证据、本地原生工具规划恢复证据（`llm_status=available`、`plan_source=react_iterated`），以及 `154` 后端备份存在性确认。剩余阻塞项已收敛为：真机同步/上传/AI 深链路、个别工具真实失败收口、重场景性能采样、生产发布级现场证据 |
...
| 导入任务编排 | 待验证 | 旧数据导入放在结构稳定后推进 | 有导入任务表、状态流与回放策略 | `import_jobs`、`V2ImportJobService`、`/v2/import-jobs/*` 与后台执行器均已落地；当前仍缺服务器现场与真机导入闭环证据 | 当前状态流已收口为 `failed/cancelled -> retry`、`pending/running -> cancel` | `29`、`30`、`32` |
...
| AI 助手视觉与流程贴合 | 新版已做 | `43-ai-assistant-requirements.md` 已升级为需求 / 审查基线，并新增 `AI_AGENT_P0_EVIDENCE_MATRIX.md` 把 AGT-P0-001..019 映射到现有接口、Android 设备、锁屏阻塞和性能证据；当前 Android 已补“首段回答前 pending result_block 不抢占主内容”和短暂真实工具提示口径，且 2026-07-03 已补本地真实 provider `model_stream` + audit 证据，但取消端到端、生产 profile、性能和逐屏 UI 证据仍为 partial | Agent 域页面达到设计稿级别，并通过真实 provider / 真机 / 审计证据证明 ChatGPT-like agent 体验 | `43`、`31`、android `feature/agent`、`docs/acceptance-evidence/ai-agent/AI_AGENT_P0_EVIDENCE_MATRIX.md` |
...
| 2026-07-03 | B07/B11 本地真实 provider 流式证据 + 154 备份确认 | 保持原状态（进行中记录） | `tools/export_codex_llm_env.sh`、`src/main/resources/application-local.yml`、`src/main/java/com/zhihuiji/backend/infrastructure/repository/AgentRunAuditEventRepository.java`、`src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java`、`src/main/java/com/zhihuiji/backend/application/service/v2/agent/component/RunAuditService.java`、`src/test/java/com/zhihuiji/backend/infrastructure/repository/RepositoryOwnerScopeAuditTest.java`、`web/src/shared/utils/business.ts`、`web/src/entities/screen/live-screen-data.ts`、`docs/acceptance-evidence/ai-agent/20260703-2250-41300-3xvunYP8-manual/`、`docs/acceptance-evidence/ai-agent/20260703-codex-provider-model-stream.md`、`docs/acceptance-evidence/b11/backend/20260703-154-backup-inventory.md` | `43-ai-assistant-requirements.md`、`AI_AGENT_P0_EVIDENCE_MATRIX.md`、`40-batch-development-master-plan.md`、`docs/acceptance-evidence/b11/20260703-strict-audit-snapshot.md` | 本轮把当前 `~/.codex/config.toml` 的真实 provider 配置接入本地后端，跑出真实 `answer_delta(delta_source=model_stream)` SSE，并用 audit 对账确认 `mode=tool_query_llm_streamed`、`llm_status=streaming`、`event_count=34`；同时补了 owner-scope 审计门禁测试、Web live-screen-data 共享 helper 去重收口，并把 `154` 主机后端备份存在性确认写入证据。剩余 AI 阻塞已收敛为 provider 原生工具规划兼容性、真机配对证据、异常分支和发布级现场。 |
...
5. AI 助手当前已有独立需求 / 审查基线：`43-ai-assistant-requirements.md` 和 `AI_AGENT_P0_EVIDENCE_MATRIX.md` 明确了真实 agentic、无模拟数据、真实查询、真实工具提示、Markdown / result block、provider stream、取消、审计和性能的 P0 门禁；2026-07-03 已补到本地真实 provider `model_stream` 证据，但现有证据整体仍以 `partial` 为主，不能替代取消端到端、生产 profile、性能和最新真机 UI 证据。
6. B11 当前已完成矩阵、脚本入口与大量本地自动化/构建复验；服务器只读现状也已确认当前真实生产拓扑为 `124` 公网边缘 + `154` 应用主机，且 `154` 后端备份存在。2026-07-03 本轮执行 `./gradlew clean test --rerun-tasks` 真实重跑 `BUILD SUCCESSFUL`（7 executed，非 UP-TO-DATE），三端编译（后端/Android/Web）全部通过。但同步/导入、媒体上传、真机 AI 深链路、性能、安全发布与生产业务现场仍不得仅凭本地编译、只读探针或局部 smoke 升级为 `新版已做`。
7. 下一步应优先解决剩余阻塞：真机深链路与 Android 配对证据、provider 原生工具规划与异常分支兼容性、重场景性能采样、生产发布级现场证据。不要再把“主体已实现”误写成“全部完成”。
