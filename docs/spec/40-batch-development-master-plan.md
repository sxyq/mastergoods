...
| B06 同步与导入链路 | 待验证 | `/v2/sync/*` 与 `/v2/import-jobs/*` 首轮已落地；本轮已复核受 B05 影响的数据对象，并补 ` /v1/sync pay_order.account_id ` 兼容映射 |
| B07 媒体与 AI 扩域 | 待验证 | media/agent 后端首轮合同已落地，已补会话更新/删除、状态约束与级联删除，仍待真实上传链与工作台联调 |
...
| B09 安卓 feature `/v2` 迁移 | 待验证 | 商品、伙伴、单据、财务、报表、助手、设置 feature 层已切到 /v2 数据链；本地编译与 V2 契约测试通过；真机联调待验证 |
| B10 安卓 UI 重构与设计稿贴合 | 待验证 | 已按 B09 UI 审核修复商品详情路径、报表时间区间真实刷新、容器壳页顶栏母版统一，以及档案/单据/财务列表的胶囊主操作；本轮继续把 Agent 工作台/任务/通知/草稿页的假数据占位收口为明确待联调空态，并修正草稿页 tab/计数/展示不一致；同时把 dashboard/reports 的误导性本地聚合语义进一步降级为诚实态说明；最新定向 Kotlin 编译已复核通过，但 dashboard/reports 视觉贴合与真机逐页截图核对仍未完成 |
| B11 联调、测试、性能、安全、发布验收 | 待验证 | 验收矩阵、证据目录约定与本地复验脚本入口已建立；后端/Android 定向本地自动化已通过并写入证据目录，且本轮已补档 5 条本地复验原始日志；Android repository 委派层已额外补齐 `product/customer/supplier/order/sync` 的定向单测，但仍不能把这些局部/定向覆盖误写成真机或发布验收完成；本轮仅补强可静态说明的 dashboard/reports/agent 本地验收边界，不假装完成真机/117 现场验证；当前宿主已明确缺少 `adb` / `emulator`，因此真机 UI smoke 需切换环境继续；Android release 与 117 发布静态清单已补齐，且 `assembleRelease`、`bootJar` 已通过 |
...
| B11 | 联调、测试、性能、安全、发布验收 | 待验证 | 后端、安卓、真机、117 环境统一验收；当前先建立验收矩阵与可复验命令入口 | `32`、`41`、测试报告、发布文档 |
...
| 安卓 `/v2` 真机联调 | 待验证 | 117 环境、登录、业务链、导入、同步、报表、助手都要跑；当前宿主已确认缺少 `adb` / `emulator` | 有可复验的测试记录和截图 | `32`、`41`、android technical-analysis |
...
6. B11 当前已完成矩阵、脚本入口与本地自动化复验；后端 smoke、Android contract、Android assemble、Android assembleRelease、backend bootJar，以及新增的 Android `repository` 委派定向单测均已通过并补档代码状态摘要；本轮又补了五条与最新 UI 收口直接相关的证据：一条是 `dashboard/reports/agent/app` 的定向 Kotlin 编译，一条是 `finance/app` 的定向 Kotlin 编译，一条是 `products/customers/suppliers/sales/purchases/payments/app` 的详情/编辑页诚实态定向 Kotlin 编译，一条是最新整包 `android-assemble` 复验，一条是最新整包 `android-assemble-release` 复验，但真机、117、性能、安全发布仍不得仅凭编译或局部单测成功升级为 `新版已做`。
7. 下一步应优先切到具备 `adb` / `emulator` 或真机接入的环境，继续真机/117 联调与截图验收；不要再把“主体已实现”误写成“全部完成”。
