...
| 工具查询只取前 10 条或最近列表 | 回答会把截断数据说成全量结论 | 后端 `tool_completed` 已补 `total_count`、`returned_count`、`limit`、`is_truncated` 和 `duration_ms`，Android 已接收并在 RunTrace 展示；仍需补 `next_cursor` / evidence / query window | 构造超过 10 条数据，确认 UI 和回答标注截断 |
...
| 工具事件字段仍未满足完整合同 | RunTrace 难以证明真实查询范围、截断、耗时和失败边界 | started / completed / failed 已存在，completed 已补数量 / 截断 / 耗时并被 Android 消费；仍缺完整 input summary、started_at/completed_at、evidence、query window、next_cursor、统一 trace_id | SSE 日志必须包含工具事件序列和字段完整性证据 |
...
- `03-tool-results.json`：每个工具输入摘要、查询窗口、结果摘要、截断和耗时。
- `04-ui-home.png`、`05-ui-chat.png`、`06-ui-runtrace.png`、`07-ui-blocks.png`：真机截图。
- `08-ui-tree.xml`：`uiautomator` dump。
- `09-forbidden-scan.txt`：禁止项扫描和逐项解释。
- `10-latency.md`：首事件、首 token / 首摘要、工具、模型、端到端耗时。
- `11-conclusion.md`：按 `pass` / `fail` / `partial` 写结论。
...
| `src/main/java/com/zhihuiji/backend/application/service/v2/V2AgentAiService.java:268-352` 当前 stream 会创建真实 `runId` 并发送 `run_started`、安全检查、最终完成事件；`V2AgentAiService.java:359-393` 会按计划执行工具并捕获失败。 | 已具备 run lifecycle 雏形，但 event envelope 尚缺 `event_id`、`seq`、`trace_id`，也缺服务端 run 状态存储。 | 部分支撑 AGT-P0-002、AGT-P0-012；仍需补统一 envelope、服务端审计和可观测性字段。 |
| `V2AgentAiService.java:1361-1390` 已发送 `tool_started` / `tool_completed`；`V2AgentAiService.java:1393-1409` 已发送 `tool_failed`，且 `V2AgentAiService.java:370-380` 会在工具异常时进入失败事件路径。 | 工具失败事件路径已存在；completed payload 已补 `duration_ms`、`returned_count`、`total_count`、`limit`、`is_truncated`，Android `ToolCompleted` / `ToolCallRecord` / `ToolAuditRecord` 已接收并在 RunTrace 工具卡显示；仍缺完整 `input_summary`、`started_at/completed_at`、evidence、query window、next_cursor 和统一 trace_id。 | 部分通过 AGT-P0-004、AGT-P0-006；下一步要补完整工具合同字段和运行验收。 |
| `V2AgentAiService.java` 仅在模型 streaming 回调内发送 `answer_delta(delta_source=model_stream)`；LLM disabled 或 stream failed / empty 时不再发送规则摘要分块，而是通过 `answer_completed` 返回 `tool_query_rule_summary` 和对应 `llm_status`。`V2AgentAiServiceTest.streamFallbackAnswerCompletesRuleSummaryWithoutFakeDeltas`、`streamDisabledModelAnswerCompletesRuleSummaryWithoutFakeDeltas` 覆盖 fallback / disabled 的 `answer_delta` 数量为 0、`answer_completed` 含规则摘要和降级状态。 | 后端“固定切完整答案 / 规则摘要冒充流式”的代码边界已收口；Android 只在收到服务端 `answer_delta` 时标记真实增量。当前仍不能把它视为端到端通过，因为还缺真实模型 `model_stream` 抓包和真机 UI 证据。 | AGT-P0-005 后端和 Android 展示边界已改善；仍需 SSE 抓包、真机 UI、真实模型 delta 和 rule_summary 完成态截图验收。 |
...
| `master-goods-android/core/network/src/main/java/com/zhihuiji/core/network/AgentSseClient.kt` 只连接 `/v2/agent/chat/stream` 并解析服务端 SSE / ndjson；当前已用 cancellable execute 将 Flow 取消传递到 OkHttp `Call.cancel()`，并由 `AgentSseClientCancellationTest` 覆盖“停止接收会取消底层网络调用”。`AgentStreamModels.kt` 定义工具、失败、answer_completed / answer_delta 等事件模型。 | Android 网络层方向正确，不主动生成事件；P0 停止接收不再只改 UI 状态，也会释放本地长连接资源。但事件模型仍需跟服务端统一 envelope 同步扩展，服务端 run cancel 仍是 P1。 | 支撑 AGT-P0-005、AGT-P0-006 和 P0 取消诚实态；依赖后端 envelope 和字段补齐。 |
