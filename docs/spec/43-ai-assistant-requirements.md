...
| `frontend/android/feature/agent/DEVELOPMENT.md` 与 `frontend/android/data/agent/DEVELOPMENT.md` 只保留 Agent 当前未完成边界；已实现的 Android 模块说明已删除，后续审查以源码、测试台账和本文档为正本。 | 减少过时的模块实现清单，避免把已接入能力误写成未开始或把局部通过误写成完整验收。 | 降低后续审查误判风险。 |
